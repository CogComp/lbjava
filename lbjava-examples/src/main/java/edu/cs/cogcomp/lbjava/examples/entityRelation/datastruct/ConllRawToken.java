 ...
 */
package edu.cs.cogcomp.lbjava.examples.entityRelation.datastruct;
...
public class ConllRawToken {
    ...
    public boolean equals(Object o) {
        ...
        if (sentId != that.sentId)
            return false;
        if (wordId != that.wordId)
            return false;
        if (POS != null ? !POS.equals(that.POS) : that.POS != null)
            return false;
        if (entType != null ? !entType.equals(that.entType) : that.entType != null)
            return false;
        if (phrase != null ? !phrase.equals(that.phrase) : that.phrase != null)
            return false;
        if (relatedTokens != null ? !relatedTokens.equals(that.relatedTokens)
                : that.relatedTokens != null)
            return false;
        if (s != null ? !s.equals(that.s) : that.s != null)
            return false;
        if (!Arrays.equals(splitPOS, that.splitPOS))
            return false;
        if (!Arrays.equals(splitWords, that.splitWords))
            return false;
    ...
    public ConllRelation getparteners(ConllRawToken e) {
        for (int i = 0; i < s.relations.size(); i++) {
            if (s.relations.elementAt(i).e1.wordId == e.wordId
                    || s.relations.elementAt(i).e2.wordId == e.wordId) {
    ...
    public void printInstance() {
        System.out.println("sent: " + sentId + " wordId: " + wordId + " phrase: " + phrase
                + " POS: " + POS + " entity type: " + entType);
    }

    @Override
    public String toString() {
        return "ConllRawToken{" + "sentId=" + sentId + ", wordId=" + wordId + ", entType='"
                + entType + '\'' + ", phrase='" + phrase + '\'' + '}';
    }
}
