...
public class ConllRawSentence {
    ...
    public String returnRelationType(int word1, int word2) {
        for (int i = 0; i < relations.size(); i++) {
            ...
            // relations.elementAt(i).wordId1+" word1= "+word1+" word id2 = " +
            // relations.elementAt(i).wordId2+" word2 = "+word2);
    ...
    public ConllRawToken[] returnWindow(int loc, int offset1, int offset2) {
        // String[] window=new String[(Math.min(sentTokens.size()-1,loc+offset2) - Math.max(0,
        // loc-offset1))];
        int index = 0;
        ConllRawToken[] window =
                new ConllRawToken[(Math.min(sentTokens.size() - 1, loc + offset2) - Math.max(0, loc
                        - offset1))];
        for (int i = -offset1; i <= offset2; i++) {
            if ((i == 0) || (i + loc < 0))
                continue;
            if (i + loc > sentTokens.size() - 1)
                break;
    ...
    public boolean equals(Object o) {
        ...
        if (sentId != that.sentId)
            return false;
        if (entityIndices != null ? !entityIndices.equals(that.entityIndices)
                : that.entityIndices != null)
            return false;
        if (relations != null ? !relations.equals(that.relations) : that.relations != null)
            return false;
        if (sentTokens != null ? !sentTokens.equals(that.sentTokens) : that.sentTokens != null)
            return false;
