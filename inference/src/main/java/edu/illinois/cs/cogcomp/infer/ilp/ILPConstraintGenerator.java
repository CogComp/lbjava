...
public abstract class ILPConstraintGenerator {
  ...
  /**
   * Should the constraint be added as a cutting plane constraint (if the
   * inference is with CuttingPlaneILPSolver)?
   */
  ...
  /**
   * The input that this constraint operates upon need not be the full input
   * to the problem. This function  returns the part of the input that is relevant to this
   * constraint. By default, it it returns null. The only way in which it
   * will not be null is if setConstraintInput is used to explicitly set the
   * input.
   *
   * @return
   */
  ...
  public void setConstraintOutputGenerator(ITransformer<IStructure,
          IStructure> transformer) {
