...
public class DiscreteConjunctiveFeature extends DiscreteFeature {
     ...
     **/
    public DiscreteConjunctiveFeature(String p, String c, DiscreteFeature l, DiscreteFeature r) {
     ...
     *
     * @param c The classifier from which package and name information is taken.
     * @param l One feature argument.
     * @param r The other feature argument.
     **/
    public DiscreteConjunctiveFeature(Classifier c, DiscreteFeature l, DiscreteFeature r) {
        this(c.containingPackage, c.name, l, r);

        if (c.allowableValues().length == 0)
            valueIndex = -1;
        else {
     ...
     *
     * @return The empty string, since conjunctive features don't have identifiers.
     **/
     ...
     *
     * @return {@link ByteString#emptyString}, since conjunctive features don't have identifiers.
     **/
    ...
    public ByteString getByteStringValue() {
        ...
        ByteString[] toAppend = {new ByteString("&", b.getEncoding()), right.getByteStringValue()};
    ...
    public boolean valueEquals(String v) {
        return getStringValue().equals(v);
    }
    ...
    public Feature getFeatureKey(Lexicon lexicon, boolean training, int label) {
        if (!training)
            return this;
        return new DiscreteConjunctiveFeature(containingPackage, generatingClassifier,
                getArgumentKey(left, lexicon, label), getArgumentKey(right, lexicon, label),
    ...
    /**
     * Returns a {@link RealConjunctiveFeature} with exactly the same children as this feature.
     **/
    public RealFeature makeReal() {
        return new RealConjunctiveFeature(containingPackage, generatingClassifier, left, right);
    }
    ...
    public Feature encode(String e) {
        ...
        if (newLeft == left && newRight == right)
            return this;
        return new DiscreteConjunctiveFeature(containingPackage, generatingClassifier, newLeft,
                newRight, valueIndex, totalValues);
    }
    ...
    /**
     * Returns a hash code based on the hash codes of {@link #left} and {@link #right}.
     *
     ...
     * @return Integers appropriate for sorting features first by {@link #left} and then by
     *         {@link #right}.
     **/
    ...
    public void read(ExceptionlessInputStream in) {
        super.read(in);
        left = (DiscreteFeature) readFeature(in);
        right = (DiscreteFeature) readFeature(in);
    }
