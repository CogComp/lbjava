...
public class DiscreteReferringStringFeature extends DiscreteReferrer {
    /**
     * The <code>identifier</code> string distinguishes this <code>Feature</code> from other
     * <code>Feature</code>s.
     **/
     ...
     **/
    public DiscreteReferringStringFeature(Classifier c, String i, DiscreteFeature r) {
        this(c.containingPackage, c.name, i, r, r.getValueIndex(), r.totalValues());
    }
     ...
     * @param i The new referring feature's identifier.
     * @param r The discrete feature referred to by this new feature.
     * @param av The allowable values of the classifier that produced <code>r</code>.
     **/
    public DiscreteReferringStringFeature(Classifier c, String i, DiscreteFeature r, String[] av) {
        super(c, r, av);
        identifier = i;
    }
     ...
     **/
    public DiscreteReferringStringFeature(String p, String c, String i, DiscreteFeature r,
            short vi, short t) {
        super(p, c, r, vi, t);
        identifier = i;
    }
    ...
    /**
     * Return the feature that should be used to index this feature into a lexicon. If it is a
     * binary feature, we return the feature with an empty value so that the feature will be mapped
     * to the same weight whether it is active or not. If the feature can take multiple values, then
     * simply return the feature object as-is.
     *
    ...
    public Feature getFeatureKey(Lexicon lexicon, boolean training, int label) {
        DiscreteFeature f = (DiscreteFeature) referent.getFeatureKey(lexicon, training, label);
        if (training)
            f = (DiscreteFeature) lexicon.getChildFeature(f, label);
        else if (f == referent)
            return this;
        return new DiscreteReferringStringFeature(containingPackage, generatingClassifier,
                identifier, f, valueIndex, totalValues);
    }


    /**
     * Returns a {@link RealPrimitiveFeature} whose {@link RealPrimitiveFeature#value value} field
     * is set to the strength of the current feature, and whose {@link #identifier} field contains
     * all the information necessary to distinguish this feature from other features.
     **/
    public RealFeature makeReal() {
        return new RealReferringStringFeature(containingPackage, generatingClassifier, identifier,
                referent.makeReal());
    }
    ...
    public Feature withStrength(double s) {
        ...
        if (f == null)
            return null;
        return new DiscreteReferringStringFeature(containingPackage, generatingClassifier,
                identifier, f, (short) Math.round(f.getStrength()), totalValues);
    }
    ...
    public Feature encode(String e) {
        ...
        if (referent == newReferent && (e == null || e == "String"))
            return this;
        ByteString id =
                identifier.length() == 0 ? ByteString.emptyString : new ByteString(identifier, e);
        return new DiscreteReferringFeature(containingPackage, generatingClassifier, id,
                newReferent, valueIndex, totalValues);
    }


    /**
     * The hash code of a <code>DiscreteReferringStringFeature</code> is the sum of the hash codes
     * of its containing package, identifier, and the referent feature.
     *
    ...
    /**
     * Two <code>DiscreteReferringStringFeature</code>s are equivalent when their containing
    ...
    public boolean equals(Object o) {
        return super.equals(o)
                && (o instanceof DiscreteReferringStringFeature ? identifier
                        .equals(((DiscreteReferringStringFeature) o).identifier) : identifier
                        .equals(((DiscreteReferringFeature) o).identifier))
    ...
    public int compareNameStrings(Object o) {
        ...
        if (d != 0)
            return d;
        return identifier.compareTo(((DiscreteReferringStringFeature) o).identifier);
    }
