...
import edu.cs.cogcomp.lbjava.Main;
...
import edu.cs.cogcomp.lbjava.Train;
import edu.cs.cogcomp.lbjava.infer.Inference;
import edu.cs.cogcomp.lbjava.infer.ParameterizedConstraint;


/**
 * A symbol table is simply a <code>HashMap</code> associating names with their types. This class
 * also assumes responsibility for determining type information for externally defined names.
 *
 * <p>
 * The global symbol table also keeps track of <code>package</code> and <code>import</code>
...
public class SymbolTable {
     ...
     * absolute path to the file with the same name and the given extension. This method was written
     * for use by the LBJava compiler and its generated code. The user should not need to call it.
     *
     * @param name The name of the class.
     * @param extension The extension of the file to search for.
     * @param path Paths in which to search for name.extension.
     * @return An object representing the file or <code>null</code> if it can't be located.
     **/
     ...
     * @return The type previously associated with the given name or <code>null</code> if no type
     *         was previously associated with it.
     **/
    public Type put(Argument a) {
        return put(a.getName(), a.getType());
    }
    ...
    /**
     * Retrieves the type associated with the given name. If this table does not contain the name,
     * imported packages are searched.
     *
     * @param name The name to retrieve type information for.
     * @return The type associated with the given name or <code>null</code> if no type information
     *         could be found.
     **/
    ...
    public Type get(String name) {
        ...
        if (c != null) {
            if (ParameterizedConstraint.class.isAssignableFrom(c)) {
                ...
                try {
                    constraint = (ParameterizedConstraint) c.newInstance();
                } catch (Exception e) {
                    System.err.println("Can't instantiate parameterized constraint '" + c
                            + "'.  Make sure there is a public, no argument "
                            + "constructor defined:");
                    e.printStackTrace();
                    System.exit(1);
                }

                result = new ConstraintType(Type.parseType(constraint.getInputType()));
                ...
                result =
                        new ClassifierType(Type.parseType(classifier.getInputType()),
                                new ClassifierReturnType(classifier.getOutputType(),
                ...
                try {
                    inference = (Inference) c.newInstance();
                } catch (Exception e) {
                ...
                for (int i = 0; i < headFinderTypeStrings.length; ++i)
                    headFinderTypes[i] = new ReferenceType(new Name(headFinderTypeStrings[i]));
    ...
    /**
     * Attempts to locate the named class in the current package and any imported packages. If the
     * corresponding Java source file is found and either the class file does not exist or its time
     * of last modification is earlier than the java file's, it is recompiled. If no class with the
    ...
    public Class<?> classForName(Name name) {
        if (parent != null)
            return parent.classForName(name);
        ...
        for (Iterator<String> I = prefixes.iterator(); I.hasNext() && result == null;) {
            ...
            if (javaFile != null) {
                ...
                if ((classFile == null || javaFile.lastModified() > classFile.lastModified())
                        && Train.runJavac(javaFile.toString()))
                    System.exit(1);
            }

            try {
                result = Class.forName(fqName);
            } catch (Exception e) {
     ...
     *
     * @param key The name.
     * @return <code>true</code> iff <code>key</code> is already a key in this table or any of its
     *         parents.
     **/
     ...
     *
     * @param key The name.
     * @return <code>true</code> iff <code>key</code> is already a key in this table.
     **/
    ...
    /**
     * Generates <code>package</code> and <code>import</code> statements from the names in the
     * member variable <code>imported</code>.
     *
    ...
    /**
     * Prints this table and all its children recursively to <code>STDOUT</code>.
     **/
    public void print() {
        print("");
    }
    ...
    public void print(String indent) {
        ...
        for (int i = 0; i < symbols.length; ++i)
            System.out.println(indent + "  " + symbols[i] + " -> " + table.get(symbols[i]));
