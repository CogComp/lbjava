...
import edu.cs.cogcomp.lbjava.classify.DiscreteConjunctiveFeature;
import edu.cs.cogcomp.lbjava.classify.DiscreteReferrer;
import edu.cs.cogcomp.lbjava.classify.Feature;
import edu.cs.cogcomp.lbjava.classify.RealConjunctiveFeature;
import edu.cs.cogcomp.lbjava.classify.RealReferrer;

/**
 * Instances of this class are intended to store features that are children of other features and
 * which do not correspond to their own weights in any learner's weight vector. While a
 * {@link Lexicon} will store an instance of this class in its {@link Lexicon#lexiconChildren}
 * field, an instance of this class will never do so. Also, the {@link #lookupChild(Feature)} method
 * behaves differently in this class, since it is assumed that children are stored here.
 **/
public class ChildLexicon extends Lexicon {
    /**
     * The elements of this vector (which correspond to the features in {@link #lexiconInv}) serve a
     ...
     * by sign if the corresponding feature has been marked for removal.
     **/
     ...
     * @return The representation of <code>f</code> that used to be stored here, or
     *         <code>null</code> if it wasn't present.
     **/
    public Feature remove(Feature f) {
        if (contains(f)) { // contains(Feature) calls lazyMapCreation()
            ...
            if (count == 0) {
                ...
                System.err
                        .println("LBJava ERROR: Marking feature as removable for the second time: "
                                + f);
                new Exception().printStackTrace();
                System.exit(1);
            }
    ...
    /**
     * The parent of feature <code>f</code> is being removed, so we decrement <code>f</code>'s
     * parent counts and remove it if it's ready.
     *
     * @param f The child feature whose parent counts need updating and which may be removed as
     *        well.
     **/
    public void decrementParentCounts(Feature f) {
        ...
        if (count == 0) {
            System.err.println("LBJava ERROR: Parent count incorrect for feature " + f);
            new Exception().printStackTrace();
            System.exit(1);
        } else if (count < 0) {
    ...
    /**
     * Updates the counts in {@link #parents} for the children of <code>f</code>.
     *
     * @param f The feature to look up.
     * @param label The label of the example containing this feature, or -1 if we aren't doing per
     *        class feature counting.
    ...
    /**
     * Helper method for methods like {@link #childLexiconLookup(DiscreteConjunctiveFeature,int)}
    ...
    /**
     * Unlike the overridden method in {@link Lexicon}, this method simply checks {@link #lexicon}
    ...
    public void write(ExceptionlessOutputStream out) {
        ...
        int n = 0;
        while (n < size && lexiconInv.get(n) != null)
            ++n;
        int i = n;
        while (i < size && lexiconInv.get(i) == null)
            ++i;
        while (i < size) {
    ...
    /**
     * <!-- read(ExceptionlessInputStream) --> Reads a binary representation of the lexicon.
     *
