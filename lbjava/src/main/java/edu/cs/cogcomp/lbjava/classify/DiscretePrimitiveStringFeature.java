...
/**
 * This feature is functionally equivalent to {@link DiscretePrimitiveFeature}, however its
 * {@link #value} is stored as a <code>String</code> instead of a {@link ByteString}. Discrete
 * classifiers return features of this type (or {@link DiscreteConjunctiveFeature}s or
 * {@link DiscreteReferringFeature}s that contain features of this type). Before storing these
 * features in a lexicon, however, they are converted to {@link DiscretePrimitiveFeature}s using the
 * specified encoding.
 *
 * @author Nick Rizzolo
 **/
public class DiscretePrimitiveStringFeature extends DiscreteFeature {
    ...
    /**
     * Sets both the identifier and the value. The value index and total allowable values, having
     * not been specified, default to -1 and 0 respectively.
     *
     ...
     **/
    public DiscretePrimitiveStringFeature(String p, String c, String i, String v, short vi, short t) {
        super(p, c, vi, t);
        identifier = i;
        value = v;
    }
    ...
    public Feature getFeatureKey(Lexicon lexicon, boolean training, int label) {
        if (totalValues() == 2)
            return new DiscretePrimitiveStringFeature(containingPackage, generatingClassifier,
                    identifier, "", (short) -1, (short) 2);
        return this;
    }
    ...
    public RealFeature makeReal() {
        if (totalValues == 2)
            return new RealPrimitiveStringFeature(containingPackage, generatingClassifier,
    ...
    public Feature withStrength(double s) {
        if (totalValues != 2 || !(s == 0 || s == 1))
            return null;
        return new DiscretePrimitiveStringFeature(containingPackage, generatingClassifier,
                identifier, "", (short) Math.round(s), (short) 2);
    }
    ...
    public Feature encode(String e) {
        ...
        return new DiscretePrimitiveFeature(containingPackage, generatingClassifier, id,
                new ByteString(value, e), valueIndex, totalValues);
    }


    /**
     * The hash code of a <code>DiscretePrimitiveStringFeature</code> is the sum of the hash codes
    ...
    /**
     * Two <code>DiscretePrimitiveStringFeature</code>s are equivalent when their containing
    ...
    public boolean equals(Object o) {
        ...
        return f.identifier.equals(identifier) && valueIndex > -1 ? valueIndex == f.valueIndex
