 ...
 * {@link ClassifierName}s and {@link LearningClassifierExpression}s.
 *
 * @author Nick Rizzolo
 **/
public class ClassifierCSE extends Pass {
    /**
     * Maps each classifier expression to the canonical name that will represent it.
     **/
     ...
     **/
    public ClassifierCSE(AST ast) {
        super(ast);
    }


    /**
     * Looks up the given expression in {@link #expressionToName}, returning a new
     * {@link ClassifierName} if there was a name associated with it or <code>null</code> otherwise.
     * In the case that a name was not already associated with the given expression, its own name is
     * set up in association with it, with the following exceptions. {@link ClassifierName}s are
     * excluded from the map since they are already merely names.
     * {@link LearningClassifierExpression}s are also excluded, since each learning classifier
     * expression should represent a separate and independent learned function even if its
     * specification is identical to some other learning classifier.
     *
    ...
    private ClassifierName lookup(ClassifierExpression ce) {
        ...
        if (cached == null) {
            if (!(ce instanceof ClassifierName) && !(ce instanceof ClassifierCastExpression)
                    && !(ce instanceof LearningClassifierExpression)) {
                cached = new ClassifierName(ce.name.toString(), ce.line, ce.byteOffset);
    ...
    public void run(ClassifierCastExpression cce) {
        ...
        if (name != null) {
            if (cce.expression.name.toString().indexOf("$$") == -1) {
                ...
                SemanticAnalysis.representationTable.remove(cce.name.toString());
    ...
    public void run(LearningClassifierExpression lce) {
        ...
        if (lce.labeler != null) {
            ...
            if (name != null) {
                SemanticAnalysis.representationTable.remove(lce.labeler.name.toString());
                lce.labeler = name;
            }
        }
        ...
        if (name != null) {
            SemanticAnalysis.representationTable.remove(lce.extractor.name.toString());
            lce.extractor = name;
        }
    }
