...
public class ClassifierReturnType extends Type {
     ...
     * </code>
     **/
    private static final String[] typeNames = {"discrete", "real", "mixed", "discrete[]", "real[]",
            "mixed[]", "discrete%", "real%", "mixed%"};
    ...
    /**
     * The index of the type represented by this <code>ClassifierReturnType</code>.
     **/
    public int type;
    /**
     * (&not;&oslash;) If the type is DISCRETE, this variable represents a list of legal values.
     **/
     ...
     **/
    public ClassifierReturnType(String s) {
        this(s, new ConstantList());
    }
    ...
    public ClassifierReturnType(String s, ConstantList l) {
        ...
        values = l;
        type = 0;
        while (type < typeNames.length && !s.equals(typeNames[type]))
            ++type;
        assert type < typeNames.length : "Couldn't find type name: " + s;
    }

    /**
     * Default constructor. Line and byte offset information, having not been supplied, are set to
     * -1.
     *
     * @param t The index of the primitive type.
     **/
    public ClassifierReturnType(int t) {
        this(t, new ConstantList());
    }
     ...
     * @param l The list of legal values.
     **/
    public ClassifierReturnType(int t, ConstantList l) {
        this(t, l, -1, -1);
    }
     ...
     *
     * @param crt The type of the classifier to which features are hypothetically being added.
     * @return <code>true</code> iff this type is more specific than the specified type.
     **/
    public boolean isContainableIn(ClassifierReturnType crt) {
        if (crt.values.size() > 0) {
            ...
            if (!constants.contains(new Constant("*"))) {
                for (ConstantList.ConstantListIterator I = values.listIterator(); I.hasNext();) {
                    Constant c = I.nextItem();
                    if (!c.value.equals("*") && !constants.contains(c))
                        return false;
                }
            }
        }

        return type == crt.type || crt.type == MIXED_GENERATOR || type == DISCRETE
                && (crt.type == DISCRETE_ARRAY || crt.type == DISCRETE_GENERATOR)
                || type == DISCRETE_ARRAY && crt.type == DISCRETE_GENERATOR || type == REAL
                && (crt.type == REAL_ARRAY || crt.type == REAL_GENERATOR) || type == REAL_ARRAY
                && crt.type == REAL_GENERATOR;
    }


    /**
     * Returns an object representing the <code>class</code> that this type represents.
     *
     * @return An object representing the <code>class</code> that this type represents.
     **/
     ...
     *
     * @param o The <code>Object</code> whose equality with this object needs to be tested.
     * @return <code>true</code> if the two <code>Object</code>s are equal, and <code>false</code>
     *         otherwise.
     **/
    public boolean equals(Object o) {
        return o instanceof ClassifierReturnType && type == ((ClassifierReturnType) o).type;
    }
     ...
     **/
    public Object clone() {
        return new ClassifierReturnType(type, values);
    }
