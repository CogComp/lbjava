...
public class DiscreteArrayStringFeature extends DiscretePrimitiveStringFeature {
    ...
    /**
     * Sets the identifier, value, array index, and size of the containing array. The value index
     * and total allowable values, having not been specified, default to -1 and 0 respectively.
     *
     ...
     **/
    public DiscreteArrayStringFeature(String p, String c, String id, String v, int i, int l) {
     ...
     * @param l The length of the array this feature is contained in.
     **/
    public DiscreteArrayStringFeature(String p, String c, String id, String v, short vi, short t,
            int i, int l) {
        super(p, c, id, v, vi, t);
        arrayIndex = i;
        arrayLength = l;
    ...
    /**
     * Return the feature that should be used to index this feature into a lexicon. If the feature
     * is binary, we return the feature with an empty value so that it will be mapped to the same
     * weight whether it is active or not. If the feature can take multiple values, then simply
     * return it as-is.
     *
    ...
    public Feature getFeatureKey(Lexicon lexicon, boolean training, int label) {
        if (totalValues() == 2)
            return new DiscreteArrayStringFeature(containingPackage, generatingClassifier,
                    identifier, "", (short) -1, (short) 2, getArrayIndex(), 0);
        return this;
    }


    /**
     * Returns a {@link RealArrayFeature} whose {@link RealPrimitiveFeature#value value} field is
     * set to the strength of the current feature, and whose {@link #identifier} field contains all
     * the information necessary to distinguish this feature from other features.
     **/
    public RealFeature makeReal() {
        if (totalValues == 2)
            return new RealArrayStringFeature(containingPackage, generatingClassifier, identifier,
                    valueIndex, arrayIndex, arrayLength);
        else {
            StringBuffer id = new StringBuffer(identifier);
            id.append('_');
            id.append(value);
            return new RealArrayStringFeature(containingPackage, generatingClassifier,
                    id.toString(), 1, arrayIndex, arrayLength);
        }
    }
    ...
    public Feature withStrength(double s) {
        if (totalValues != 2 || !(s == 0 || s == 1))
            return null;
        return new DiscreteArrayStringFeature(containingPackage, generatingClassifier, identifier,
    ...
    public Feature encode(String e) {
        ...
        return new DiscreteArrayFeature(containingPackage, generatingClassifier, id,
                new ByteString(value, e), valueIndex, totalValues, arrayIndex, arrayLength);
    }


    /**
     * The hash code of a <code>DiscreteArrayStringFeature</code> is the sum of the hash codes of
     * the containing package, the identifier, the value and the array index.
     *
     * @return The hash code of this <code>Feature</code>.
     **/
    public int hashCode() {
        return 31 * super.hashCode() + arrayIndex;
    }


    /**
     * Two <code>DiscreteArrayStringFeature</code>s are equivalent when their containing packages,
    ...
    public boolean equals(Object o) {
        return super.equals(o)
                && (o instanceof DiscreteArrayStringFeature ? arrayIndex == ((DiscreteArrayStringFeature) o).arrayIndex
