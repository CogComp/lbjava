 ...
 * capable of predicting multiple labels on new examples. A separate {@link LinearThresholdUnit} is
 * learned independently to predict whether each label is appropriate for a given example. Any
 * {@link LinearThresholdUnit} may be used, so long as it implements its <code>clone()</code> method
 * and a public constructor that takes no arguments. During testing, the {@link #classify(Object)}
 * method returns a separate feature for each {@link LinearThresholdUnit} whose score on the example
...
public class MultiLabelLearner extends SparseNetworkLearner {
     ...
     * {@link MultiLabelLearner.Parameters} object.
     *
     * @param p The settings of all parameters.
     **/
    public MultiLabelLearner(Parameters p) {
        this("", p);
    }
     ...
     **/
    public MultiLabelLearner(String n) {
        super(n);
    }
    ...
    /**
     * Returns a separate feature for each {@link LinearThresholdUnit} whose score on the example
     * object exceeds the threshold.
     *
    ...
    public FeatureVector classify(int[] exampleFeatures, double[] exampleValues) {
        ...
        for (int i = 0; i < network.size(); ++i) {
            ...
            if (score >= 0)
                result.addFeature(predictions.get(i));
        }
    ...
    /**
     * Simply a container for all of {@link MultiLabelLearner}'s configurable parameters. Using
