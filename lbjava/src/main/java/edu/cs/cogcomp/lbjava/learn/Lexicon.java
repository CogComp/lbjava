...
import edu.cs.cogcomp.lbjava.util.ByteString;
import edu.cs.cogcomp.lbjava.util.ClassUtils;
import edu.cs.cogcomp.lbjava.util.FVector;
import edu.cs.cogcomp.lbjava.util.TableFormat;
import edu.illinois.cs.cogcomp.core.datastructures.vectors.*;


/**
 * A <code>Lexicon</code> contains a mapping from {@link Feature}s to integers. The integer key of a
 * feature is returned by the {@link #lookup(Feature)} method. If the feature is not already in the
 ...
 *
 * <p>
 * The map is implemented as a <code>HashMap</code> by default and the <code>Lexicon</code> class
 * has similar functionality. This class also maintains a second <code>Vector</code> of integers to
 * their associated features for fast reverse lookup using the {@link #lookupKey(int)} method.
 *
 * @author Michael Paul
 **/
public class Lexicon implements Cloneable, Serializable {
    ...
    /**
     * <!-- readLexicon(String) --> Reads and returns a feature lexicon from the specified file.
     *
    ...
    public static Lexicon readLexicon(String filename) {
        try {
            return readLexicon(new URL("file:" + filename));
        } catch (Exception e) {
    ...
    /**
     * <!-- readLexicon(URL) --> Reads a feature lexicon from the specified location.
     *
    ...
    /**
     * <!-- readLexicon(URL,boolean) --> Reads a feature lexicon from the specified location, with
     * the option to ignore the feature counts by setting the second argument to <code>false</code>.
     *
     * @param url The location from which to read the feature lexicon.
    ...
    /**
     * <!-- readLexicon(ExceptionlessInputStream,boolean) --> Reads a feature lexicon from the
     * specified stream.
     *
     ...
     * @return The lexicon.
     **/
    public static Lexicon readLexicon(ExceptionlessInputStream in, boolean readCounts) {
    ...
    /**
     * This flag remembers whether {@link #encoding} has been assigned a value yet or not. Using
    ...
    /**
     * Features at this index in {@link #lexiconInv} or higher have been pruned. <code>-1</code>
     * indicates that no pruning has been done.
     **/
    ...
    public void setEncoding(String e) {
        if (encodingSet && (encoding == null ? e != null : !encoding.equals(e))) {
            System.err
                    .println("LBJava ERROR: Once established, the encoding of a lexicon cannot be "
                            + "changed.");
    ...
    public int size() {
        return lexiconInv.size();
    }

    /**
     * Returns the value of {@link #pruneCutoff}, or {@link #size()} if {@link #pruneCutoff} is -1.
     **/
    public int getCutoff() {
        return pruneCutoff == -1 ? size() : pruneCutoff;
    }


    /**
     * <!-- countFeatures(CountPolicy) --> Call this method to initialize the lexicon to count
     * feature occurrences on each call to <code>lookup(feature, true)</code> (counting still won't
     * happen on a call to <code>lookup(feature, false)</code>). Alternatively, this method can also
     * cause the lexicon to discard all its feature counts and cease counting features at any time
     * in the future. The former happens when <code>policy</code> is something other than
     * {@link Lexicon.CountPolicy#none}, and the latter happens when <code>policy</code> is
     * {@link Lexicon.CountPolicy#none}.
     *
    ...
    /**
     * <!-- getCountPolicy() --> Returns the feature counting policy currently employed by this
     * lexicon.
     **/
    ...
    /**
     * <!-- perClassToGlobalCounts() --> Collapses per-class feature counts into global counts.
     **/
    public void perClassToGlobalCounts() {
        ...
        for (int j = 0; j < columns; ++j) {
            int count = 0;
            for (int i = 0; i < rows; ++i)
                count += perClassFeatureCounts.get(i, j);
    ...
    /**
     * <!-- contains(Feature) --> Returns <code>true</code> if the given feature is already in the
     * lexicon (whether it's past the {@link #pruneCutoff} or not) and <code>false</code> otherwise.
     * This does not alter or add anything to the lexicon.
     *
     * @param f The feature to look up.
     * @return A boolean indicating if the given feature is currently in the lexicon.
     **/
    ...
    /**
     * <!-- lookup(Feature) --> Looks up a feature's index by calling <code>lookup(f, false)</code>.
    ...
    /**
     * <!-- lookup(Feature,boolean,int) --> Looks up the given feature in the lexicon, possibly
     * counting it and/or expanding the lexicon to accomodate it. Feature counting and automatic
     * lexicon expansion happen when <code>training</code> is <code>true</code>. Otherwise,
     * <code>f</code> is not counted even if already in the lexicon, and a previously unobserved
     * feature will cause this method to return the value of {@link #getCutoff()} without expanding
     * the lexicon to accomodate the new feature.
     *
     * @param f The feature to look up.
     * @param training Whether or not the learner is currently training.
     ...
     * method checks {@link #lexiconChildren} if the feature isn't present in {@link #lexicon} and
     * {@link #lexiconInv}, and then stores the given feature in {@link #lexiconChildren} if it
     * wasn't present anywhere.
     *
     * @param f The feature to look up.
     * @param label The label of the example containing this feature, or -1 if we aren't doing per
     *        class feature counting.
     * @return A feature equivalent to <code>f</code> that is stored in this lexicon.
     **/
    public Feature getChildFeature(Feature f, int label) {
        ...
        if (lexiconChildren == null)
            lexiconChildren = new ChildLexicon(this);
     ...
     * @param label The label of the example containing this feature, which is ignored if we aren't
     *        doing per class feature counting.
     **/
    protected void incrementCount(int index, int label) {
        if (featureCounts != null)
            featureCounts.increment(index);
     ...
     * this method checks {@link #lexiconChildren} if the feature isn't present in {@link #lexicon}
     * and {@link #lexiconInv}, and will throw an exception if it still can't be found.
     *
     * @param f The feature to look up.
     * @return If the feature was found in {@link #lexicon}, its associated integer index is
     *         returned. Otherwise, <code>-i - 1</code> is returned, where <code>i</code> is the
     *         index associated with the feature in {@link #lexiconChildren}.
     * @throws UnsupportedOperationException If the feature isn't found anywhere in the lexicon.
     **/
    ...
    /**
     * <!-- lookupKey(int) --> Does a reverse lexicon lookup and returns the {@link Feature}
     * associated with the given integer key, and <code>null</code> if no such feature exists.
     *
     * @param i The integer key to look up. If <code>i</code> is negative, {@link #lexiconChildren}
     *        is queried instead of {@link #lexiconInv}.
     * @return The feature that maps to the given integer.
     **/
    public Feature lookupKey(int i) {
        if (i < 0)
            return lexiconChildren.lookupKey(-i - 1);
        return lexiconInv.get(i);
    }


    /**
     * <!-- isPruned(int,PruningPolicy) --> Determines if the given feature index should be pruned
     * according to the given pruning policy, which must have its thresholds set already in the case
     * that it represents the "Percentage" policy. This method behaves equivalently to
     * <code>isPruned(i, -1, p)</code>.
     *
    ...
    /**
     * <!-- isPruned(int,int,PruningPolicy) --> Determines if the given feature index should be
     * pruned according to the given pruning policy, which must have its thresholds set already in
     * the case that it represents the "Percentage" policy. The second argument to this method
     * represents the label of the example in which the specified feature appeared. It is ignored
     * unless per class feature counts are present. If they are, then when the specified label is
     * -1, all counts for the given feature must be greater than or equal to the corresponding
     * threshold for this method to return <code>true</code>. When per class feature counts are
     ...
     * the lexicon as in {@link #prune(Lexicon.PruningPolicy)}. Passing a non-negative label in the
     * second argument gives the behavior expected when pruning the actual examples.
     *
     * @param i The feature index.
     * @param label The label of the example containing this feature, or -1 if we want the lexicon
     *        pruning behavior.
     * @param policy The pruning policy.
     * @return <code>true</code> iff the feature should be pruned.
     **/
    public boolean isPruned(int i, int label, PruningPolicy policy) {
        ...
        if (label >= 0)
            return perClassFeatureCounts.get(label, i) < policy.getThreshold(label);
    ...
    /**
     * <!-- prune(PruningPolicy) --> Rearranges the order in which features appear in the lexicon
     * based on the compiled feature counts in {@link #featureCounts} or
     * {@link #perClassFeatureCounts} so that pruned features are at the end of the feature space.
     ...
     * the features' original indexes to their new ones after pruning. The {@link #getCutoff()}
     ...
     * represents the first feature whose count fell below the threshold for <i>every</i> class.
     ...
     * @return A map from features' original indexes to their new ones, or <code>null</code> if
     *         <code>policy</code> indicates no pruning.
     **/
    public int[] prune(PruningPolicy policy) {
        ...
        if (featureCounts == null && perClassFeatureCounts == null)
            throw new UnsupportedOperationException(
                    "LBJava ERROR: Lexicon.prune: Can't prune if there's no feature " + "counts.");

        // Set thresholds in the policy.
        if (policy.isPercentage()) {
            if (featureCounts != null) { // if global counting
                long t = Math.round(Math.ceil(featureCounts.max() * policy.getPercentage()));
                ...
                for (int i = 0; i < size; ++i)
                    thresholds[i] = (int) Math.round(Math.ceil(perClassFeatureCounts.max(i) * p));
        ...
        // threshold must already be established in that case.
            ...
            throw new UnsupportedOperationException("LBJava ERROR: Lexicon.prune: Pruning policy '"
        ...
        // needs to be pruned.
        if (pruneCutoff > 0)
            swapMap[pruneCutoff - 1] = pruneCutoff - 1;
        ...
        for (int i = pruneCutoff - 2; i >= 0; --i) {
            if (isPruned(i, policy)) {
                ...
                if (featureCounts != null)
                    featureCounts.set(i, featureCounts.set(pruneCutoff, featureCounts.get(i)));
                else {
                    for (int j = 0; j < perClassFeatureCounts.size(); ++j)
    ...
    public boolean equals(Object o) {
        if (!o.getClass().equals(getClass()))
            return false;
        Lexicon l = (Lexicon) o;
        return pruneCutoff == l.pruneCutoff
                && (lexicon == null ? l.lexicon == null : lexicon.equals(l.lexicon))
                && (featureCounts == null ? l.featureCounts == null : featureCounts
                        .equals(l.featureCounts))
                && (perClassFeatureCounts == null ? l.perClassFeatureCounts == null
                        : perClassFeatureCounts.equals(l.perClassFeatureCounts))
                && (lexiconChildren == null ? l.lexiconChildren == null : lexiconChildren
                        .equals(l.lexiconChildren));
    }


    /** Returns a hash code for this lexicon. */
    public int hashCode() {
        return lexiconInv.hashCode();
    }


    /**
     * <!-- write(ExceptionlessOutputStream) --> Writes a binary representation of the lexicon.
     *
    ...
    public void write(ExceptionlessOutputStream out) {
        ...
        if (featureCounts == null)
            ...
            featureCounts.write(out);
        if (perClassFeatureCounts == null)
            out.writeInt(0);
        else
            perClassFeatureCounts.write(out);
    }


    /**
     * <!-- read(ExceptionlessInputStream) --> Reads the binary representation of a lexicon from the
    ...
    /**
     * <!-- read(ExceptionlessInputStream,boolean) --> Reads the binary representation of a lexicon
     * from the specified stream, overwriting the data in this object. This method also gives the
     * option to ignore any feature counts stored after the feature mappings by setting the second
    ...
    public void read(ExceptionlessInputStream in, boolean readCounts) {
        ...
        if (readCounts) {
            ...
            if (perClassFeatureCounts.size() == 0)
                perClassFeatureCounts = null;
        } else {
            featureCounts = null;
            perClassFeatureCounts = null;
        ...
        if (lexiconChildren != null)
            lexiconChildren.setParent(this);
    ...
    /**
     * Various other methods in this class call this method to ensure that {@link #lexicon} is
     * populated before performing operations on it. The only reason it wouldn't be is if it had
     * just been read off disk.
     **/
    ...
    /**
     * <!-- readPrunedSize(ExceptionlessInputStream) --> Reads the value of {@link #pruneCutoff}
    ...
    /**
     * <!-- printCountTable(boolean) --> Produces on <code>STDOUT</code> a table of feature counts
     * including a line indicating the position of {@link #pruneCutoff}. It's probably not a good
     * idea to call this method unless you know your lexicon is small.
     *
    ...
    public void printCountTable(boolean p) {
        ...
        TableFormat
                .printTableFormat(System.out, columnLabels, rowLabels, data, sigDigits, dashRows);
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            filename = args[0];
            if (args.length == 2)
                p = Boolean.parseBoolean(args[1]);
            if (args.length > 2)
                throw new Exception();
        } catch (Exception e) {
            System.out
                    .println("usage: java edu.cs.cogcomp.lbjava.learn.Lexicon <lex file> [<package names = true>]");
            System.exit(1);
        }
    ...
    /**
     * <!-- class CountPolicy --> Immutable type representing the feature counting policy of a
     * lexicon. When LBJava's self imposed restriction to use Java 1.4 is lifted, this class will be
     * replaced by an <code>enum</code>.
     *
     * <p>
     ...
     * <dt><b>Global</b></dt>
     * <dd>
     * The lexicon stores one integer count per feature, and every occurrence of the feature adds to
     * this count regardless of the example it appears in.</dd>
     * <dt><b>Per Class</b></dt>
     * <dd>
     * The lexicon stores one integer count for each (feature, prediction class) pair. When a given
     * feature appears in example, this occurrence adds to the count associated with the example's
     * label, assuming that examples have a single discrete label.</dd>
     * </dl>
     * </blockquote>
     *
     * @author Nick Rizzolo
    ...
    /**
     * <!-- class PruningPolicy --> Represents the feature counting policy of a lexicon. Objects of
     * this type are used to identify and describe a desired pruning policy. In particular, the
     * description of a pruning policy includes feature count thresholds which sometimes need to be
     * computed in terms of data. Space is allocated within objects of this type for storing these
     * thresholds whenever they are computed.
     *
     * <p>
     ...
     * <dt><b>Absolute</b></dt>
     * <dd>
     * Features whose counts within a given dataset fall below an absolute threshold are pruned from
     * that dataset.</dd>
     * <dt><b>Percentage</b></dt>
     * <dd>
     * Features whose counts within a given dataset are lower than a given percentage of the most
     * common feature's count are pruned from that dataset.</dd>
     * </dl>
     * </blockquote>
     *
     * @author Nick Rizzolo
    ...
    public static class PruningPolicy {
        ...
        /** The names of the different counting policies as strings. */
        private static final String[] names = {"none", "absolute", "percentage"};
        ...
        /**
         * Feature count thresholds which may either be specified by the policy explicitly or
         * computed in terms of data.
         **/
        ...
        /**
         * Returns the value of the <code>i</code><sup>th</sup> threshold in {@link #thresholds}
         * when in "Percentage" mode, but ignores the parameter <code>i</code> and returns the first
         * element of {@link #thresholds} when in "Absolute" mode.
         *
         * @param i An index.
