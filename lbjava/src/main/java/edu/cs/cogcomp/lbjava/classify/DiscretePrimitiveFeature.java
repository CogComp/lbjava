...
public class DiscretePrimitiveFeature extends DiscreteFeature {
     ...
     **/
    public DiscretePrimitiveFeature(String p, String c, ByteString i, ByteString v) {
     ...
     **/
    public DiscretePrimitiveFeature(String p, String c, ByteString i, ByteString v, short vi,
            short t) {
        super(p, c, vi, t);
        identifier = i;
        value = v;
    ...
    public Feature getFeatureKey(Lexicon lexicon, boolean training, int label) {
        if (totalValues() == 2)
            return new DiscretePrimitiveFeature(containingPackage, generatingClassifier,
                    identifier, ByteString.emptyString, (short) -1, (short) 2);
        return this;
    }
    ...
    public RealFeature makeReal() {
        if (totalValues == 2)
            return new RealPrimitiveFeature(containingPackage, generatingClassifier, identifier,
                    valueIndex);
        else {
            ByteString id = (ByteString) identifier.clone();
            ByteString[] toAppend = {new ByteString("_", id.getEncoding()), value};
            id.append(toAppend);
            return new RealPrimitiveFeature(containingPackage, generatingClassifier, id, 1);
        }
    }
    ...
    public Feature withStrength(double s) {
        if (totalValues != 2 || !(s == 0 || s == 1))
            return null;
        return new DiscretePrimitiveFeature(containingPackage, generatingClassifier, identifier,
    ...
    /**
     * The hash code of a <code>DiscretePrimitiveFeature</code> is the sum of the hash codes of its
     * containing package, identifier, and value.
     *
    ...
    /**
     * Two <code>DiscretePrimitive(String)Feature</code>s are equivalent when their containing
