 ...
 * representation of it. The resulting string(s) can be pure ASCII or latex compatible.
 *
 * @author Nick Rizzolo
 **/
public class TableFormat {
    ...
    /**
     * Simply converts the type of the given matrix from <code>double</code> to <code>Double</code>.
     *
     * @param m The matrix.
    ...
    /**
     * Formats the given data into an ASCII table and prints it to the given stream. All non-
     * <code>null</code> entries will be rounded to three significant digits after the decimal
     * place.
     *
     * @param out The stream.
    ...
    /**
     * Formats the given data into an ASCII table. All non-<code>null</code> entries will be rounded
     * to three significant digits after the decimal place.
     *
     * @param data The numerical data.
     ...
     *
     * @param out The stream.
     * @param columnLabels One label for each column in the output, starting with the column of row
     *        labels if one exists. If <code>null</code>, no column label row will be returned.
     * @param rowLabels One label for each non-<code>null</code> row in <code>data</code>. If
     *        <code>null</code>, no row label column will be returned.
     * @param data The numerical data.
     **/
    public static void printTableFormat(PrintStream out, String[] columnLabels, String[] rowLabels,
            double[][] data) {
     ...
     * @return The string respresentation of the data.
     **/
    public static String[] tableFormat(String[] columnLabels, String[] rowLabels, double[][] data) {
        if (data == null || data.length == 0)
            return new String[0];
    ...
    /**
     * Formats the given data into an ASCII table and prints it to the given stream. Any entry in
     * any row of <code>data</code> set to <code>null</code> will become a single dash in the
     * output. All non-<code>null</code> entries will be rounded to three significant digits after
     * the decimal place.
     *
     * @param out The stream.
     * @param columnLabels One label for each column in the output, starting with the column of row
    ...
    /**
     * Formats the given data into an ASCII table. Any entry in any row of <code>data</code> set to
     * <code>null</code> will become a single dash in the output. All non-<code>null</code> entries
     * will be rounded to three significant digits after the decimal place.
     *
     * @param columnLabels One label for each column in the output, starting with the column of row
     *        labels if one exists. If <code>null</code>, no column label row will be returned.
     * @param rowLabels One label for each non-<code>null</code> row in <code>data</code>. If
     ...
     *        <code>null</code>, no row label column will be returned.
     * @param data The numerical data.
     * @param sigDigits Significant digits specified on a column by column basis, only for
     *        <code>data</code> columns.
     **/
    public static void printTableFormat(PrintStream out, String[] columnLabels, String[] rowLabels,
     ...
     * @param sigDigits Significant digits specified on a column by column basis, only for
     *        <code>data</code> columns.
     * @param dashRows The indexes of rows in <code>data</code> which should be preceded by a row of
     *        dashes in the output.
     **/
    public static void printTableFormat(PrintStream out, String[] columnLabels, String[] rowLabels,
            double[][] data, int[] sigDigits, int[] dashRows) {
        printTable(out, tableFormat(columnLabels, rowLabels, data, sigDigits, dashRows));
    }
    ...
    public static String[] tableFormat(String[] columnLabels, String[] rowLabels, double[][] data,
        ...
        if (data == null || data.length == 0)
            return new String[0];
        return tableFormat(columnLabels, rowLabels, wrapDouble(data), sigDigits, dashRows);
    }
    ...
    public static String[] tableFormat(String[] columnLabels, String[] rowLabels, Double[][] data,
        ...
        if (data == null || data.length == 0)
            return new String[0];
        if (sigDigits == null)
            return tableFormat(columnLabels, rowLabels, data);
        ...
        if (columnLabels != null) {
            if (columnLabels.length < columns) {
                ...
                System.arraycopy(columnLabels, 0, temp, columns - columnLabels.length,
                        columnLabels.length);
                columnLabels = temp;
            }
        ...
        return asciiTableFormat(columns, dataColumns, rows, dataRows, columnLabels, rowLabels,
                data, sigDigits, dashRows);
    }


    /**
     * Formats the given data into an ASCII table. Any row of <code>data</code> set to
     * <code>null</code> will become a row with one dash in each cell of the output table's row. Any
     * entry in any row of <code>data</code> set to <code>null</code> will also become a single dash
     * in the output.
     *
     ...
     * @param columnLabels One label for each column in the output, starting with the column of row
     *        labels if one exists. If <code>null</code>, no column label row will be returned.
     * @param rowLabels One label for each non-null row in <code>data</code>. If <code>null</code>,
     *        no row label column will be returned.
     * @param data The numerical data.
     * @param sigDigits Significant digits specified on a column by column basis, only for
     *        <code>data</code> columns.
     * @param dashRows The indexes of rows in <code>data</code> which should be preceded by a row of
     ...
     * @return The string respresentation of the data.
     **/
    protected static String[] asciiTableFormat(int columns, int dataColumns, int rows,
            int dataRows, String[] columnLabels, String[] rowLabels, Double[][] data,
            int[] sigDigits, int[] dashRows) {
        ...
        for (int i = 0; i < dataRows; ++i)
            for (int j = 0; j < dataColumns; ++j)
                table[dataRowStart + i][dataColumnStart + j] =
                        data[i] == null || data[i][j] == null ? "-" : format(
                                data[i][j].doubleValue(), sigDigits[j]);
        ...
        if (rowLabels != null)
            for (int i = 0; i < dataRows; ++i)
                table[dataRowStart + i][0] = ljust(table[dataRowStart + i][0], columnWidths[0]);

        for (int i = 0; i < dataRows; ++i)
            for (int j = 0; j < dataColumns; ++j)
                table[dataRowStart + i][dataColumnStart + j] =
                        table[dataRowStart + i][dataColumnStart + j].equals("-") ? center("-",
                                columnWidths[dataColumnStart + j]) : rjust(
                                table[dataRowStart + i][dataColumnStart + j],
                                columnWidths[dataColumnStart + j]);

        Arrays.sort(dashRows);
        int d = 0;
        while (d < dashRows.length && dashRows[d] < 0)
            ++d;
     ...
     * @param d The number of significant digits to round to. If less than 0, this method assumes 0;
     *        if greater than 18, this method assumes 18.
    ...
    protected static String format(double f, int d) {
        ...
        while (buffer.length() < d + leftDigits)
            buffer.insert(0, '0');
        if (d > 0)
            buffer.insert(buffer.length() - d, '.');
     ...
     * @param width The width within which the original string should be padded.
     * @return The padded string. If the original is longer than the specified width, the original
     *         is simply returned.
     **/
