...
import edu.cs.cogcomp.lbjava.frontend.Yylex;
...
import edu.cs.cogcomp.lbjava.IR.DeclarationList;
...
import edu.cs.cogcomp.lbjava.io.HexStringInputStream;


/**
 * To be run after <code>SemanticAnalysis</code>, this pass determines which
 * <code>CodeGenerator</code>s need to have their code generated and which classifiers need to be
 * trained based on the revisions made to the LBJava source file.
 *
 * <p>
 * A hard coded classifier, a constraint, or an inference named <code>foo</code> needs its code
 * regenerated iff at least one of the following is true:
 * <ul>
 * <li>The file <code>foo.java</code> does not exist.
 * <li>
 * Using the comments at the top of <code>foo.java</code>, it is determined that the code specifying
 * <code>foo</code> has been revised.
 * </ul>
 * If the comments at the top of <code>foo.java</code> do not exist, or if they don't have the
 * expected form, the file will not be overwritten and an error will be generated.
 *
 * <p>
 * All <code>CodeGenerator</code>s are also labeled as either "affected" (by a revision) or
 * "unaffected". An <code>CodeGenerator</code> named <code>foo</code> is labeled "affected" iff at
 * least one of the following is true:
 * <ul>
 * <li>
 * <code>foo</code> is a hard coded classifier, a constraint, or an inference and either:
 * <ul>
 * <li>its code needed to be regenerated as described above or
 * <li>it invokes another "affected" <code>CodeGenerator</code>.
 * </ul>
 * <li>
 * <code>foo</code> is a learning classifier and at least one of its label or extractor classifiers
 * is "affected".
 * </ul>
 *
 * <p>
 * A learning classifier named <code>foo</code> needs to have its code regenerated and retrained iff
...
public class RevisionAnalysis extends Pass {
    ...
    public static final String[] statusNames = {"unaffected", "affected", "revised"};
    ...
    /**
     * Set to <code>true</code> iff no code has changed since the compiler was last run.
     **/
    ...
    public static void printRevisionStatus() {
        ...
        for (Iterator I = revisionStatus.entrySet().iterator(); I.hasNext();) {
            ...
            Object classifierExpression = SemanticAnalysis.representationTable.get(name);
            if (classifierExpression instanceof LearningClassifierExpression) {
                ...
                System.out.println("  features: " + statusToString(lce.featuresStatus));
                System.out.println("  pruning: " + statusToString(lce.pruneStatus));
                System.out.println("  learning: " + statusToString(lce.learningStatus));
                System.out.println("  only code generation: " + lce.onlyCodeGeneration);
            }
        }
    }
    ...
    /**
     * Returns the name of a revision status, or <code>"no status"</code> if the status is
     * <code>null</code>.
     **/
     ...
     * @param line The line number at which the classifier whose source we're reading is declared in
     *        its LBJava source file.
     * @return The second line from the classifier's generated code without the opening comment
     *         marker (//), or <code>null</code> if the generated code doesn't exist or the file
     *         doesn't appear to be generated code.
     **/
    private static String readSecondLine(String name, int line) {
        ...
        try {
            in = new BufferedReader(new FileReader(javaSource));
        } catch (Exception e) {
        ...
        if (line1 == null || line2 == null || !line2.startsWith("// ")
            ...
            reportError(line, "The file '" + name + "' does not appear to have been generated by "
                    + "LBJava, but LBJava needs to overwrite it.  Either remove the file, "
    ...
    /**
     * This method reads the comments at the top of the file containing the code corresponding to
     * the specified code generating node to determine if the LBJava source describing that code
     * generator has been modified since the LBJava compiler was last executed.
     *
     * @param node The code generating node.
     * @param convert Whether or not the code is converted to hexadecimal compressed format.
     * @return <code>true</code> iff the associated Java file did not exist or it contained the
     *         expected comments and those comments indicate that a revision has taken place.
     **/
    private static boolean codeRevision(CodeGenerator node, boolean convert) {
        ...
        if (convert) {
            ...
            expected = node.shallow().toString();
    ...
    private static void propagateAffected(String name) {
        boolean isCompositeGenerator =
                SemanticAnalysis.representationTable.get(name) instanceof CompositeGenerator;
        ...
        for (Iterator I = dependors.iterator(); I.hasNext();) {
            ...
            if (SemanticAnalysis.representationTable.get(dependor) instanceof LearningClassifierExpression) {
                LearningClassifierExpression lce =
                        (LearningClassifierExpression) SemanticAnalysis.representationTable
                                .get(dependor);
            ...
            if (!revisionStatus.containsKey(dependor)) {
                if (isCompositeGenerator
                        && isRevised
                        && SemanticAnalysis.representationTable.get(dependor) instanceof LearningClassifierExpression)
                    revisionStatus.put(dependor, REVISED);
                else
                    revisionStatus.put(dependor, AFFECTED);
    ...
    public void run(DeclarationList list) {
        ...
        String[] revised = (String[]) revisionStatus.keySet().toArray(new String[0]);
        for (int i = 0; i < revised.length; ++i)
            propagateAffected(revised[i]);

        for (Iterator I = SemanticAnalysis.dependorGraph.keySet().iterator(); I.hasNext();) {
            Object name = I.next();
            if (!revisionStatus.containsKey(name)) {
                revisionStatus.put(name, UNAFFECTED);

                if (SemanticAnalysis.representationTable.get(name) instanceof LearningClassifierExpression) {
                    LearningClassifierExpression lce =
                            (LearningClassifierExpression) SemanticAnalysis.representationTable
                                    .get(name);

                    if (lce.featuresStatus == null)
                        lce.featuresStatus = UNAFFECTED;
                    if (lce.pruneStatus == null)
                        lce.pruneStatus = UNAFFECTED;
                    if (lce.learningStatus == null)
                        lce.learningStatus = UNAFFECTED;
                }
            }
        }
    }
    ...
    /**
     * Parses a learning classifier expression out of an encoded string using the automatically
     * generated scanner and parser.
     *
     * @param s The string out of which the learning classifier expression will be parsed.
    ...
    private static LearningClassifierExpression parseLCE(String s) {
        ...
        try {
            ...
            System.err.println("LBJava ERROR: Can't instantiate string parser for LCE:");
            e.printStackTrace();
            System.exit(1);
        }
        ...
        AST ast = null;
        try {
            ast = (AST) new parser(new Yylex(reader)).parse().value;
        } catch (Exception e) {
        ...
        ClassifierAssignment ca = (ClassifierAssignment) ast.declarations.iterator().next();
    ...
    public void run(LearningClassifierExpression lce) {
        ...
        boolean preExtractToDisk =
                lce.preExtract.value.startsWith("\"disk") || lce.preExtract.value.equals("true")
        ...
        if (!oldLCE.returnType.equals(lce.returnType)
                ...
                || (oldLCE.labeler == null ? lce.labeler != null : lce.labeler == null
                        || !oldLCE.labeler.name.equals(lce.labeler.name))
                || !oldLCE.extractor.name.equals(lce.extractor.name)
                || (oldLCE.parser == null ? lce.parser != null : !oldLCE.parser.equals(lce.parser))
                || (oldLCE.featureEncoding == null ? lce.featureEncoding != null
                        : lce.featureEncoding == null
                                || !oldLCE.featureEncoding.value.equals(lce.featureEncoding.value))
                || preExtractToDisk && !previousPreExtractToDisk
                || (preExtractToDisk ? !exFile.exists() : !lcFile.exists()) || !lexFile.exists()) {
            revisionStatus.put(lceName, REVISED);
            lce.featuresStatus = lce.pruneStatus = lce.learningStatus = AFFECTED;
            return;
        }

        if ((oldLCE.pruneCountType == null ? lce.pruneCountType != null : !oldLCE.pruneCountType
                .equals(lce.pruneCountType))
                || (oldLCE.pruneThresholdType == null ? lce.pruneThresholdType != null
                        : !oldLCE.pruneThresholdType.equals(lce.pruneThresholdType))
                || (oldLCE.pruneThreshold == null ? lce.pruneThreshold != null
        ...
        if ((oldLCE.learnerName == null ? lce.learnerName != null : !oldLCE.learnerName
                .equals(lce.learnerName))
                || (oldLCE.learnerConstructor == null ? lce.learnerConstructor != null
                        : !oldLCE.learnerConstructor.equals(lce.learnerConstructor))
                || (oldLCE.learnerParameterBlock == null ? lce.learnerParameterBlock != null
                        : !oldLCE.learnerParameterBlock.toString().equals(
                                lce.learnerParameterBlock.toString()))
                || (oldLCE.K == null ? lce.K != null : !oldLCE.K.equals(lce.K))
                || oldLCE.splitPolicy != lce.splitPolicy
                || (oldLCE.testingMetric == null ? lce.testingMetric != null
                        : !oldLCE.testingMetric.equals(lce.testingMetric))
                || !oldLCE.alpha.equals(lce.alpha) || !lcFile.exists()) {
            lce.featuresStatus = lce.pruneStatus = preExtractToDisk ? UNAFFECTED : REVISED;
            lce.learningStatus = REVISED;
            revisionStatus.put(lceName, AFFECTED);
            return;
        }

        if (oldLCE.rounds == null ? lce.rounds != null : !oldLCE.rounds.equals(lce.rounds)) {
            lce.featuresStatus = lce.pruneStatus = preExtractToDisk ? UNAFFECTED : REVISED;
            lce.learningStatus = REVISED;
            revisionStatus.put(lceName, AFFECTED);

            if (lce.K == null && lce.parameterSets.size() == 0 && lce.rounds instanceof Constant
                    && oldLCE.rounds instanceof Constant) {
                int rounds =
                        lce.rounds == null ? 1 : Integer.parseInt(((Constant) lce.rounds).value);
                int oldRounds =
                        oldLCE.rounds == null ? 1 : Integer
                                .parseInt(((Constant) oldLCE.rounds).value);
                if (rounds > oldRounds)
                    lce.startingRound = oldRounds + 1;
            }
        ...
        lce.onlyCodeGeneration =
                (oldLCE.comment == null ? lce.comment != null : !oldLCE.comment.equals(lce.comment))
                        || (oldLCE.cacheIn == null ? lce.cacheIn != null : !oldLCE.cacheIn
                                .equals(lce.cacheIn))
                        || oldLCE.singleExampleCache != lce.singleExampleCache
                        || (oldLCE.evaluation == null ? lce.evaluation != null : !oldLCE.evaluation
                                .equals(lce.evaluation));
