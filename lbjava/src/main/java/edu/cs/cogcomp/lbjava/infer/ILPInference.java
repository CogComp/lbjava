...
/**
 * This class employs an {@link ILPSolver} to solve a constrained inference problem. When
 * constructing an instance of this class in an LBJava source file, use one of the constructors that
 * does <i>not</i> specify a head object. The generated code will fill in the head object
 * automatically. The other constructor parameters are used to specify the ILP algorithm and enable
 * textual output of ILP variable descriptions to <code>STDOUT</code>. Textual output of the ILP
 * problem itself is controlled by the {@link ILPSolver}.
 *
 * @author Nick Rizzolo
 **/
public class ILPInference extends Inference {
    ...
    /**
     * Used during ILP constraint generation. When a propositional constraint finishes generating
     * any ILP constraints that may be associated with it, it sets this variable to its own index.
     **/
    protected int returnIndex;
    /**
     * Used during ILP constraint generation. This flag is set iff the variable corresponding to
     * <code>returnIndex</code> is negated in its current context.
     **/
    protected boolean returnNegation;
    /**
     * Used during ILP constraint generation. This map associates each variable index with a
     * representation of the expression whose value is represented by the variable. The keys
     * associated with indexes of variables that were originally part of the inference problem are
     * <code>PropositionalVariable</code> objects. The keys associated with indexes of temporary
     * variables created during constraint translation are strings.
     **/
    protected HashMap indexMap;
    /**
     * Used during ILP constraint generation. Constraints are treated differently if they are part
     * of another constraint expression than if they are a term in the top level conjunction.
     **/
    protected boolean topLevel;
    /**
     * Verbosity level. {@link ILPInference#VERBOSITY_NONE} produces no incidental output. If set to
     * {@link ILPInference#VERBOSITY_LOW}, only timing information is printed on <code>STDOUT</code>
     * . If set to {@link ILPInference#VERBOSITY_HIGH}, information mapping the generated ILP
     * variables to the first order variables they were generated from and their settings in the ILP
     * problem's solution is printed to <code>STDOUT</code>.
     **/
     ...
     **/
    public ILPInference(ILPSolver a, int v) {
        this(null, a, v);
    }
     ...
     * @param a The ILP algorithm.
     **/
    public ILPInference(Object h, ILPSolver a) {
        this(h, a, VERBOSITY_NONE);
    }
    ...
    public void addConstraint(FirstOrderConstraint c) {
        ...
        if (constraint == null)
            ...
            constraint = new FirstOrderConjunction((FirstOrderConstraint) constraint, c);
    }
    ...
    protected void infer() throws Exception {
        ...
        for (Iterator I = variables.values().iterator(); I.hasNext();) {
            ...
            if (scores == null || scores.length == 0) {
                System.err.println("LBJava ERROR: Classifier " + v.getClassifier()
                        + " did not return any scores.  ILP inference cannot be " + "performed.");
                System.exit(1);
            }
            ...
            for (int j = 0; j < scores.length; ++j) {
                indexMap.put(new PropositionalVariable(v.getClassifier(), v.getExample(),
                        scores[j].value), new Integer(indexes[j]));
                ...
                if (verbosity >= VERBOSITY_HIGH) {
                    ...
                    toPrint.append("(");
                    toPrint.append(exampleToString(v.getExample()));
                    toPrint.append(") == ");
     ...
     * @return The current value of the requested variable. If the variable does not exist in this
     *         inference, the result of the <code>Learner</code>'s
     *         <code>discreteValue(Object)</code> method applied to the <code>Object</code> is
     *         returned.
     **/
    ...
    /**
     * Two <code>Inference</code> objects are equal when they have the same run-time type and store
     * the same head object. I.e., the <code>==</code> operator must return <code>true</code> when
     * comparing the two head objects for this method to return <code>true</code>.
     *
     * @param o The object to compare to this object.
     * @return <code>true</code> iff this object equals the argument object as defined above.
     **/
    public boolean equals(Object o) {
        if (!(o instanceof ILPInference))
            return false;
     ...
     *
     * @param d A textual description of the subexpression whose value is represented by the new
     *        variable.
    ...
    public void visit(PropositionalConjunction c) {
        PropositionalConstraint[] children = (PropositionalConstraint[]) c.getChildren();
        ...
        if (topLevel) {
            PropositionalConstraint[] variables = new PropositionalConstraint[children.length];
            int size = 0;
            ...
            if (I == null) {
                ...
                solver.addLessThanConstraint(indexes, coefficients, bound + children.length - 1);
            }
    ...
    public void visit(PropositionalDisjunction c) {
        ...
        if (topLevel) {
            ...
            if (subConstraintIndex > -1) {
                ...
                int multiplier =
                        children[subConstraintIndex] instanceof PropositionalConjunction ? subChildren.length
    ...
    public void visit(PropositionalAtLeast c) {
        ...
        if (topLevel)
            ...
            if (I == null) {
                ...
                coefficients[children.length] = -children.length;
                solver.addLessThanConstraint(indexes, coefficients, bound + c.getM() - 1);
            }
    ...
    public void visit(PropositionalNegation c) {
        assert c.constraint instanceof PropositionalVariable : "ILP: Negation of a "
                + c.constraint.getClass().getName() + " encountered.";
