...
/**
 * Represents a set of possible parameters, used when doing parameter-tuning. The list of possible
 * values for a parameter is stored in {@link #parameterList} as an <code>ExpressionList</code>.
 *
 * <p>
 * The set can be defined either as a comma-separated list of possible values, or it can be defined
 * as a range in terms of a start value, end value, and increment factor, which can then be
 * converted into the explicit list of possible values.
 *
 * <p>
 * The LBJava syntax for defining the parameter set is to declare the possible values inside of
 * double curly braces, either as an explicit list or as a range. The following two examples are
 * equivalent:
 * <ul>
...
public class ParameterSet extends Expression {
    ...
    /**
     * The name of the parameter that will be printed in method signatures in the generated code.
     **/
    ...
    public Type type;
    /**
     * <code>true</code> iff this parameter set appears inside the <code>rounds</code> clause of a
     ...
     * @param i The increment factor.
     **/
    public ParameterSet(int line, int byteOffset, Expression s, Expression e, Expression i) {
        super(line, byteOffset);
        start = s;
        end = e;
        increment = i;
    ...
    /**
     * Converts this parameter set's {@link #start}, {@link #end}, and {@link #increment}
     * expressions (which must represent {@link Constant}s of a {@link PrimitiveType} other than
     * <code>boolean</code>) into an explicit list of values. The {@link #type} field must be set
    ...
    public void convertRange() {
        ...
        if (!(type instanceof PrimitiveType))
            throw new IllegalArgumentException(
                    "Can't call ParameterSet.convertRange() when type isn't " + "primitive");
        ...
        if (pt.type == PrimitiveType.CHAR) {
            ...
            for (int j = s; m * j <= e; j += i)
                parameterList.add(new Constant(this.line, this.byteOffset, "" + ((char) j)));
        } else {
        ...
        /*
         * if (lastEntry != e) { Double num = new Double(e); list.add(new Constant(this.line,
         * this.byteOffset, num.toString())); }
         */
    }


    /**
     * Parses integers out of every constant in the set and returns them in a sorted array. If this
     * parameter set was specified as a range, it is assumed that {@link #convertRange()} has
     * already been called.
     **/
    ...
    /**
     * Assuming that {@link #convertRange()} has already been called (if necessary) and that every
     * expression in {@link #parameterList} is a {@link Constant}, this method produces an array of
     * <code>String</code>s containing the values of the constants. The return type of the method is
     * <code>Object[]</code> so that its elements can be replaced by objects of other types, which
     * is convenient during parameter tuning.
     **/
    public Object[] toStringArray() {
        Object[] values = new Object[parameterList.size()];
        ExpressionList.ExpressionListIterator I = listIterator();
        for (int i = 0; I.hasNext(); ++i)
            values[i] = ((Constant) I.next()).value;
        return values;
    }
    ...
    public boolean equals(Object o) {
        ...
        return start == null ? parameterList.equals(p.parameterList) : start.equals(p.start)
                && end.equals(p.end) && increment.equals(p.increment);
    }


    /** A hash code based on the hash codes of the constituent expressions. */
    public int hashCode() {
        return start == null ? parameterList.hashCode() : 31 * start.hashCode() + 23
                * end.hashCode() + 17 * increment.hashCode();
    }
    ...
    public Object clone() {
        ParameterSet c =
                new ParameterSet(this.line, this.byteOffset, (ExpressionList) parameterList.clone());
        c.start = start;
        c.end = end;
