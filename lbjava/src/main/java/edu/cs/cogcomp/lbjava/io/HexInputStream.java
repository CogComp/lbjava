...
/**
 * This class receives input from another <code>InputStream</code> assuming that data is little
 ...
 * another <code>InputStream</code>. For instance: <br>
 * <br>
 *
 * <pre>
...
public class HexInputStream extends InputStream {
    ...
    /**
     * The <code>InputStream</code> from which yet-to-be-converted input should be received.
     **/
     ...
     *
     * @param i The input stream from which yet-to-be-converted input should be received.
     **/
    ...
    /**
     * Reads the next byte of data from the input stream. The value is returned as an
     * <code>int</code> in the range 0 to 255. If no byte is available because the end of the stream
    ...
    public int read() throws IOException {
        ...
        if (i1 == -1)
            throw new IOException("HexInputStream: Invalid input character: '" + ((char) d1)
                    + "' (" + d1 + ")");
        int i2 = digits.indexOf((char) d2);
        if (i2 == -1)
            throw new IOException("HexInputStream: Invalid input character: '" + ((char) d2)
                    + "' (" + d2 + ")");
    ...
    /**
     * Reads up to <code>len</code> bytes of data from another input stream into an array of bytes.
     * An attempt is made to read as many as <code>len</code> bytes, but a smaller number may be
     * read, possibly zero. The number of bytes actually read is returned as an integer. <br>
     * <br>
     *
     ...
     * <br>
     *
     * If <code>len</code> is zero, then no bytes are read and 0 is returned; otherwise, there is an
     ...
     * @param len The amount of bytes to be received and written into the buffer.
     * @return The total number of bytes read into the buffer, or -1 if there is no more data
     ...
     * <code>true</code>, the stream somehow remembers all the bytes read after the call to mark and
     ...
     * <code>readlimit</code> bytes are read from the stream before <code>reset</code> is called.
     *
