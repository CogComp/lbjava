...
public abstract class Type extends ASTNode {
    /**
     * This method takes a Java <code>Class</code> object and generates an LBJava <code>Type</code>
     * that represents the same type.
     *
     * @param c The <code>Class</code> to "translate".
     * @return The <code>Type</code> object that represents the same type as the argument.
     **/
    public static Type parseType(Class c) {
        return parseType(c.getName());
    }


    /**
     * This method takes a Java type encoding and generates an LBJava <code>Type</code> that
    ...
    public static Type parseType(String encoding) {
        int dimensions = 0;
        while (encoding.charAt(dimensions) == '[')
            ++dimensions;
        ...
        if (dimensions > 0) {
            switch (encoding.charAt(dimensions)) {
                case 'Z':
                    result = new PrimitiveType(PrimitiveType.BOOLEAN);
                    break;
                case 'B':
                    result = new PrimitiveType(PrimitiveType.BYTE);
                    break;
                case 'C':
                    result = new PrimitiveType(PrimitiveType.CHAR);
                    break;
                case 'D':
                    result = new PrimitiveType(PrimitiveType.DOUBLE);
                    break;
                case 'F':
                    result = new PrimitiveType(PrimitiveType.FLOAT);
                    break;
                case 'I':
                    result = new PrimitiveType(PrimitiveType.INT);
                    break;
                case 'J':
                    result = new PrimitiveType(PrimitiveType.LONG);
                    break;
                case 'S':
                    result = new PrimitiveType(PrimitiveType.SHORT);
                    break;
                case 'L':
                    break;
                default:
        ...
        if (result == null) {
            ...
            if (dimensions == 0)
                referenceString = encoding;
            else
                referenceString = encoding.substring(dimensions + 1, encoding.length() - 1);

            for (int i = referenceString.indexOf('$'); i != -1; i = referenceString.indexOf('$'))
                referenceString =
                        referenceString.substring(0, i) + "." + referenceString.substring(i + 1);
    ...
    /**
     * Set <code>true</code> by <code>SemanticAnalysis</code> iff this type will be used to
     * represent the argument of a <code>QuantifiedConstraintExpression</code>.
     **/
    public boolean quantifierArgumentType;
    /**
     * Java's <code>Class</code> object defining the class that this <code>Type</code> represents.
     **/
