...
/**
 * A constraint's type is defined by what it takes as input. This class does not represent a syntax
 * that appears in the source - it is constructed only during semantic analysis.
 *
 * @author Nick Rizzolo
 **/
public class ConstraintType extends ClassifierType {
    ...
    public ConstraintType(Type i) {
        super(i, new ClassifierReturnType(ClassifierReturnType.DISCRETE, new ConstantList(
                DiscreteFeature.BooleanValues)), false);

        try {
            myClass = Class.forName("edu.cs.cogcomp.lbjava.infer.ParameterizedConstraint");
        } catch (Exception e) {
            System.err
                    .println("Class 'edu.cs.cogcomp.lbjava.infer.ParameterizedConstraint' not "
                            + "found.  Aborting.");
            System.exit(1);
        }
    }
    ...
    /**
     * Two <code>ConstraintType</code>s are equivalent when their input types match.
     *
    ...
    public boolean equals(Object o) {
        return o instanceof ConstraintType && input.equals(((ConstraintType) o).input);
    }
     ...
     **/
    public Object clone() {
        return new ConstraintType((Type) input.clone());
    }
