...
public class FirstOrderEqualityWithVariable extends FirstOrderEquality {
     ...
     **/
    public FirstOrderEqualityWithVariable(boolean e, FirstOrderVariable l, FirstOrderVariable r) {
        this(e, l, r, null);
    }
     ...
     * <code>FirstOrderEquality</code> children.
     *
    ...
    public void setQuantificationVariables(Vector o) {
        if (replacer == null) {
            System.err.println("LBJava ERROR: Attempting to set quantification variable in "
        ...
        if (!replacer.rightConstant) {
            right = new FirstOrderVariable(right.getClassifier(), replacer.getRightObject());
    ...
    public PropositionalConstraint propositionalize() {
        ...
        if (leftScores.length == 1 && rightScores.length == 1)
            return new PropositionalConstant(leftScores[0].value.equals(rightScores[0].value));

        PropositionalVariable[] leftVariables = new PropositionalVariable[leftScores.length];
        PropositionalVariable[] rightVariables = new PropositionalVariable[rightScores.length];

        int size = 0;
        for (int i = 0; i < leftScores.length; ++i) {
            boolean found = false;
            for (int j = 0; j < rightScores.length && !found; ++j) {
                if (!leftScores[i].value.equals(rightScores[j].value))
                    continue;
                found = true;
                leftVariables[size] =
        ...
        if (!equality)
            rightVariable = new PropositionalNegation(rightVariable);
        PropositionalConstraint result =
                new PropositionalDisjunction(new PropositionalNegation(leftVariables[0]),
                        rightVariable);
        if (equality)
            result =
                    new PropositionalConjunction(result, new PropositionalDisjunction(
                            new PropositionalNegation(rightVariable), leftVariables[0]));
        ...
        for (int i = 1; i < size; ++i) {
            ...
            result =
                    new PropositionalConjunction(result, new PropositionalDisjunction(
                            new PropositionalNegation(leftVariables[i]), rightVariable));
            if (equality)
                result =
                        new PropositionalConjunction(result, new PropositionalDisjunction(
                                new PropositionalNegation(rightVariable), leftVariables[i]));
        }
    ...
    /**
     * The hash code of a <code>FirstOrderEqualityWithVariable</code> is the sum of the hash codes
     * of its children plus 2.
     *
     * @return The hash code for this <code>FirstOrderEqualityWithVariable</code>.
     **/
    public int hashCode() {
        if (replacer != null)
    ...
    /**
     * Two <code>FirstOrderEqualityWithVariable</code>s are equivalent when their children are
     * equivalent in either order.
     *
     * @return <code>true</code> iff the argument is a <code>FirstOrderEqualityWithVariable</code>
     *         involving the same variables.
     **/
    public boolean equals(Object o) {
        if (!(o instanceof FirstOrderEqualityWithVariable))
            return false;
        FirstOrderEqualityWithVariable n = (FirstOrderEqualityWithVariable) o;
        return replacer == n.replacer
                && (replacer != null || replacer == null
                        && (left.equals(n.left) && right.equals(n.right) || left.equals(n.right)
                                && right.equals(n.left)));
    }
