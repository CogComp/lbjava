...
/**
 * Useful when performing <i>k</i>-fold cross validation, this parser filters the examples coming
 * from another parser. Conceptually, the examples from the original parser are first split into
 * <i>k</i> "folds" (or partitions) depending on the selected splitting policy. A particular fold is
 * then selected as the pivot, and this parser can be configured either to return all and only the
 * examples from that fold, or all and only the examples from other folds.
 *
 * <p>
 * The <i>k</i> folds are referred to by their indexes, which are 0, 1, ..., <i>k</i> - 1. This
 * index is used to select the pivot fold.
 *
...
public class FoldParser implements Parser {
    ...
    protected int fold;
    /**
     * A lower bound for an index relating to the pivot fold. The index variable in question may
     * either be {@link #exampleIndex} or {@link #shuffleIndex}.
     **/
    protected int lowerBound;
    /**
     * An upper bound for an index relating to the pivot fold. The index variable in question may
     * either be {@link #exampleIndex} or {@link #shuffleIndex}.
     **/
    ...
    /**
     * Constructor for when you don't know how many examples are in the data. Using a constructor
     * that allows specification of the number of examples in the data only saves computation when
     * the splitting policy is either sequential or random.
     *
     * @param parser The parser whose examples are being filtered.
     * @param K The total number of folds; this value is ignored if the splitting policy is manual.
     ...
     **/
    public FoldParser(Parser parser, int K, SplitPolicy split, int pivot, boolean f) {
        this(parser, K, split, pivot, f, -1);
    }

    /**
     * Constructor for when you know neither how many examples are in the data nor <i>K</i>, i.e.,
     * how many folds are in the data. This constructor can only be used when the splitting policy
     * is manual. Using a constructor that allows specification of the number of examples in the
     * data only saves computation when the splitting policy is either sequential or random.
     *
     * @param parser The parser whose examples are being filtered.
     * @param split The way in which examples are partitioned into folds.
     * @param pivot The index of the pivot fold.
     ...
     * @param pivot The index of the pivot fold.
     * @param f Whether to extract examples from the pivot.
     * @param e The total number of examples coming from <code>parser</code>, or -1 if unknown.
     **/
    public FoldParser(Parser parser, int K, SplitPolicy split, int pivot, boolean f, int e) {
        this.K = K;
        splitPolicy = split;
        fromPivot = f;
        examples = e;
        ...
        if (examples == -1
                && (splitPolicy == SplitPolicy.sequential || splitPolicy == SplitPolicy.random)) {
            ++examples;
    ...
    /**
     * Retrieves the value of {@link #K}, which may have been computed in the constructor if the
    ...
    /**
     * Sets the value of {@link #fromPivot}, which controls whether examples will be taken from the
    ...
    /**
     * Sets this parser back to the beginning of the raw data. This means arranging for all relevant
     * state variables to be reset appropriately as well, since the value of {@link #pivot} may have
     * changed.
     *
     * @see #setPivot(int)
     **/
    public void reset() {
        ...
        if (splitPolicy == SplitPolicy.sequential || splitPolicy == SplitPolicy.random) {
            lowerBound = pivot * (examples / K) + Math.min(pivot, examples % K);
            upperBound = (pivot + 1) * (examples / K) + Math.min(pivot + 1, examples % K);
        }

        if (splitPolicy == SplitPolicy.random)
            shuffleIndex = lowerBound;
        if (splitPolicy == SplitPolicy.manual)
            fold = 0;
        exampleIndex = 0;
    }
    ...
    protected boolean filter(Object example) {
        ...
        if (splitPolicy == SplitPolicy.sequential)
            return fromPivot == (exampleIndex >= lowerBound && exampleIndex < upperBound);
        if (splitPolicy == SplitPolicy.random)
            return fromPivot == (shuffleIndex < upperBound && shuffled[shuffleIndex] == exampleIndex);
    ...
    protected void increment(Object example) {
        if (example == FoldSeparator.separator) {
            ...
            if (splitPolicy == SplitPolicy.random) {
                if (shuffleIndex < upperBound && shuffled[shuffleIndex] == exampleIndex)
                    ++shuffleIndex;
            }
     ...
     *
     * TODO: When LBJava's self imposed restriction to use Java 1.4 is lifted, this class will be
     * replaced by an <code>enum</code>.
     *
     * <p>
     * The four implemented splitting strategies are described below. Note that in all cases except
     * "Manual", the size of the folds are as equal as possible, with any extra examples allocated
     * to earlier folds.
     *
     * <blockquote>
     * <dl>
     ...
     * {@link FoldSeparator} in place of an example object.</dd>
     * </dl>
     * </blockquote>
     *
     * @author Nick Rizzolo
    ...
    public static class SplitPolicy {
        ...
        /**
         * Represents the split policy in which every k<sup>th</sup> example is part of the same
         * fold.
         **/
