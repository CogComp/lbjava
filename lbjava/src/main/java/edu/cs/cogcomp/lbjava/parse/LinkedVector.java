...
/**
 * A <code>LinkedVector</code> is used to store a vector of <code>LinkedChild</code>ren which all
 * maintain links between each other and the parent <code>LinkedVector</code>.
 *
 * @see LinkedChild
 * @author Nick Rizzolo
 **/
public class LinkedVector extends LinkedChild {
    ...
    /**
     * Constructor for when only a single child from anywhere in this vector is available. It is
     * assumed that the <code>previous</code> and <code>next</code> links are filled in by every
     * child.
     *
     ...
     * @return The child removed, or <code>null</code> if there was no child at that index.
     **/
    public LinkedChild remove(int i) {
        LinkedChild before = (i - 1 < 0) ? null : (LinkedChild) children.get(i - 1);
        LinkedChild after = (i + 1 >= children.size()) ? null : (LinkedChild) children.get(i + 1);
        ...
        try {
            removed = (LinkedChild) children.remove(i);
    ...
    /**
     * Inserts the specified child into the specified index. All children that previously had index
     * greater than or equal to the specified index are shifted up one.
     *
    ...
    public boolean insert(LinkedChild c, int i) {
        ...
        c.next = (i + 1 >= children.size()) ? null : (LinkedChild) children.get(i + 1);
     ...
     * @return The child at the specified index, or <code>null</code> if there was no child at that
     *         index.
     **/
    public LinkedChild get(int i) {
        try {
            return (LinkedChild) children.get(i);
        } catch (ArrayIndexOutOfBoundsException e) {
        }
        return null;
    }
