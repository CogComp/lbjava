...
public class UniversalQuantifier extends Quantifier {
     ...
     **/
    public UniversalQuantifier(String q, Collection col, FirstOrderConstraint con) {
        super(q, col, con);
    }
    ...
    public PropositionalConstraint propositionalize() {
        ...
        for (Iterator I = collection.iterator(); I.hasNext();) {
            ...
            if (result == null)
                result = constraint.propositionalize();
            else
                result = new PropositionalConjunction(result, constraint.propositionalize());
        }

        enclosingQuantificationSettings.removeElementAt(index);
        if (result == null)
            result = new PropositionalConstant(true);
        return result;
    }


    /**
     * The hash code of a <code>UniversalQuantifier</code> is the sum of the hash codes of its
     * children.
     *
    ...
    /**
     * Two <code>UniversalQuantifier</code>s are equivalent when their children are equivalent.
     *
     * @return <code>true</code> iff the argument is an equivalent <code>UniversalQuantifier</code>.
     **/
