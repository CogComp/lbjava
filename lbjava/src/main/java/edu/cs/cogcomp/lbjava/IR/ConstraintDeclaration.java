...
/**
 * Represents the declaration of a constraint. Constraints declared in an LBJava source file are
 * mainly used to constrain inferences, but they may also be used exactly as if they were a
 * classifier with return type <code>discrete{false, true}</code>.
 *
 * @author Nick Rizzolo
 **/
public class ConstraintDeclaration extends Declaration implements CodeGenerator {
     ...
     * @param b The code block representing the constraint.
     **/
    public ConstraintDeclaration(TokenValue t, TokenValue i, Argument a, Block b) {
     ...
     **/
    public Type getType() {
        return new ConstraintType(argument.getType());
    }
     ...
     **/
    public Object clone() {
        return new ConstraintDeclaration(comment, -1, -1, (Name) name.clone(),
                (Argument) argument.clone(), (Block) body.clone());
    }
