 ...
 * Linear Classification</i>, Dredze, et.al (ICML, 2008). This algorithm envisions each parameter
 * stored in a linear threshold unit's weight vector as having been drawn independently from a
 * normal distribution with an independent mean and variance representing our estimate and
 * confidence in that parameter. Given a training example, this algorithm then tries to find new
 * values for all these means and confidences such that both of the following hold:
 *
 * <ul>
 * <li>the KL-divergence between the old and new distributions is minimized, and
 * <li>the current example is classified correctly when a weight vector is drawn according to the
 * current distributions with user-specified confidence.
 * </ul>
 *
 * <p>
 * In this implementation, the user-specified confidence parameter is a real value representing the
 * result of applying the inverse cumulative function of the normal distribution to a probability
 * (ie, a real value greater than or equal to 0 and less than or equal to 1). The inverse of the
 * normal cdf is a monotonically increasing function.
 *
 * <p>
 ...
 * an instance of {@link SparseConfidenceWeighted.Parameters Parameters} as input. The documentation
 ...
 * {@link SparseConfidenceWeighted.Parameters Parameters} class indicates the default value of the
...
public class SparseConfidenceWeighted extends LinearThresholdUnit {
    ...
    /**
     * The confidence parameter as described above; default {@link #defaultConfidence}.
     **/
     ...
     * @param c The desired confidence value.
     * @param v The desired initial variance.
     * @param vm An empty sparse weight vector of means, perhaps of an alternative subclass of
     ...
     * {@link LinearThresholdUnit#weightVector}, and {@link #variances} parameters. Make sure that
     * the references passed to the last two arguments refer to different objects.
     *
     * @param c The desired confidence value.
     * @param v The desired initial variance.
     * @param vm An empty sparse weight vector of means, perhaps of an alternative subclass of
     *        {@link SparseWeightVector}.
     * @param vv An empty sparse weight vector of variances, perhaps of an alternative subclass of
     *        {@link SparseWeightVector}.
     **/
    public SparseConfidenceWeighted(double c, double v, SparseWeightVector vm, SparseWeightVector vv) {
        this("", c, v, vm, vv);
    }

    /**
     * Initializing constructor. Sets all member variables to their associated settings in the
     * {@link SparseConfidenceWeighted.Parameters} object.
     *
     * @param p The settings of all parameters.
     **/
    public SparseConfidenceWeighted(Parameters p) {
        this("", p);
    }
     ...
     *        {@link SparseWeightVector}.
     **/
    public SparseConfidenceWeighted(String n, double c, double v, SparseWeightVector vm) {
    ...
    /**
     * Sets the {@link #confidence} member variable to the specified value.
     *
    ...
    /**
     * Sets the {@link #initialVariance} member variable to the specified value.
     *
    ...
    public void learn(int[] exampleFeatures, double[] exampleValues, int[] exampleLabels,
        ...
        Object sigmaX[] =
                variances.pairwiseMultiply(exampleFeatures, exampleValues, initialVariance, true);
        int sigmaXFeatures[] = (int[]) sigmaX[0];
        double sigmaXValues[] = (double[]) sigmaX[1];

        double v =
                FeatureVector.dot(exampleFeatures, exampleValues, sigmaXFeatures, sigmaXValues) + 1
                        / variancesBias;
        ...
        if (alpha > 0) {
            ...
            variances.scaledAdd(exampleFeatures, exampleValues, 2 * alpha * confidence);
    ...
    /**
     * This method does nothing. The entire implementation is in {@link #learn(Object)}.
     */
     ...
     * of the classifier is printed, followed by {@link #confidence} and {@link #initialVariance}.
     * Next, the annotation <code>Begin means</code> on its own line is followed by the contents of
     ...
     * variances</code> on its own line is followed by the contents of {@link #variances} and the
     * annotation <code>End variances</code> on its own line.
     *
    ...
    public Object clone() {
        ...
        try {
            clone = (SparseConfidenceWeighted) super.clone();
        } catch (Exception e) {
    ...
    /**
     * Simply a container for all of {@link SparseConfidenceWeighted}'s configurable parameters.
