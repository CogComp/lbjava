 ...
 */
package edu.cs.cogcomp.lbjava.classify;
...
import edu.cs.cogcomp.lbjava.learn.Lexicon;
import edu.cs.cogcomp.lbjava.parse.Parser;
import edu.cs.cogcomp.lbjava.util.TableFormat;


/**
 * This class is a program that can evaluate any <code>Classifier</code> against an oracle
 * <code>Classifier</code> on the objects returned from a <code>Parser</code>.
 *
 * <p>
 * Usage: <blockquote> <code>
 *     java edu.cs.cogcomp.lbjava.classify.TestDiscrete [-t &lt;n&gt;] &lt;classifier&gt;
 *                                     &lt;oracle&gt; &lt;parser&gt;
 ...
 *
 * <p>
 * <b>Options:</b> The <code>-t &lt;n&gt;</code> option is similar to the LBJava compiler's command
 * line option of the same name. When <code>&lt;n&gt;</code> is greater than 0, a time stamp is
 * printed to <code>STDOUT</code> after every <code>&lt;n&gt;</code> examples are processed.
 *
 * <p>
 * <b>Input:</b> The first three command line parameters are fully qualified class names, e.g.
 * <code>myPackage.myClassifier</code>. Next, <code>&lt;input file&gt;</code> is passed (as a
 * <code>String</code>) to the constructor of <code>&lt;parser&gt;</code>. The optional parameter
 * <code>&lt;null label&gt;</code> identifies one of the possible labels produced by
 * <code>&lt;oracle&gt;</code> as representing "no classification". It is used during the
 * computation of overall precision, recall, and F1 scores. Finally, it is also assumed that
 * <code>&lt;classifier&gt;</code> is discrete, and that its <code>discreteValue(Object)</code>
 * method is implemented.
 *
 * <p>
 * <b>Output:</b> First some timing information is presented. The first time reported is the time
 * taken to load the specified classifier's Java class into memory. This reflects the time taken for
 * LBJava to load the classifier's internal representation <b>if</b> the classifier does <b>not</b>
 * make use of the <code>cachedin</code> keyword. Next, the time taken to evaluate the first example
 * is reported. It isn't particularly informative unless the classifier <b>does</b> make use of the
 * <code>cachedin</code> keyword. In this case, it reflects the time LBJava takes to load the
 * classifier's internal representation better than the first time reported. Finally, the average
 * time taken to execute the classifier's <code>discreteValue(Object)</code> method is reported.
 *
 * <p>
 * After the timing information, an ASCII table is written to <code>STDOUT</code> reporting
 * precision, recall, and F<sub>1</sub> scores itemized by the values that either the classifier or
 * the oracle produced during the test. The two rightmost columns are named <code>"LCount"</code>
 * and <code>"PCount"</code> (standing for "labeled count" and "predicted count" respectively), and
 ...
 * recall, and F<sub>1</sub> scores and a total count of non-null-labeled examples are reported at
...
public class TestDiscrete {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Classifier loaded in " + (totalTime / 1000.0) + " seconds.");
     ...
     * provide the labeled testing data. This simplified interface to
     * {@link #testDiscrete(TestDiscrete,Classifier,Classifier,Parser,boolean,int)} assumes there
     * are no null predictions and that output should not be generated on <code>STDOUT</code>.
     *
     ...
     * @return A new <code>TestDiscrete</code> object filled with testing statistics.
     **/
    public static TestDiscrete testDiscrete(Classifier classifier, Classifier oracle, Parser parser) {
        return testDiscrete(new TestDiscrete(), classifier, oracle, parser, false, 0);
    }


    /**
     * Tests the given discrete classifier against the given oracle using the given parser to
     * provide the labeled testing data. If the parser returns examples as <code>Object[]</code>s
     * containing arrays of <code>int</code>s and <code>double</code>s, as would be the case if
     ...
     * also assumed that the given discrete classifier is an instance of <code>Learner</code> and
     * thus a lexicon of label mappings can be retrieved from it.
     *
     * @param tester An object of this class that has already been told via {@link #addNull(String)}
     *        which prediction values are considered to be null predictions.
     * @param classifier The classifier to be tested.
     * @param oracle The classifier to test against.
     * @param parser The parser supplying the labeled example objects.
     ...
     * @return The same <code>TestDiscrete</code> object passed in the first argument, after being
     *         filled with statistics.
     **/
    public static TestDiscrete testDiscrete(TestDiscrete tester, Classifier classifier,
            Classifier oracle, Parser parser, boolean output, int outputGranularity) {
        int processed = 1;
        long totalTime = 0;
        ...
        if (output && outputGranularity > 0) {
            ...
            if (example instanceof Object[] && ((Object[]) example)[0] instanceof int[]) {
                ...
                gold =
                        ((Feature) labelLexicon.lookupKey(((int[]) ((Object[]) example)[2])[0]))
                                .getStringValue();
            } else
            ...
            for (example = parser.next(); example != null; example = parser.next(), ++processed) {
                ...
                totalTime += System.currentTimeMillis();
                assert prediction != null : "Classifier returned null prediction for example "
                        + example;
    ...
    /**
     * Given command line parameters representing the fully qualified names of the classifier to be
     * tested, the oracle classifier to test against, the parser supplying the testing objects, and
     * the input parameter to the parser's constructor this method instantiates all three objects.
     *
    ...
    private static TestDiscrete instantiate(String[] args) {
        ...
        try {
            ...
            for (int i = offset + 4; i < args.length; ++i)
                result.addNull(args[i]);
        } catch (Exception e) {
            System.err
                    .println("usage:\n"
                            + "  java edu.cs.cogcomp.lbjava.classify.TestDiscrete [-t <n>] <classifier> <oracle> \\\n"
                            + "                                  <parser> <input file> \\\n"
    ...
    public void reportPrediction(String p, String l) {
        ...
        if (p.equals(l))
            histogramAdd(correctHistogram, p, 1);
    }
     ...
     *
     * @return An array containing the predictions that have been reported so far.
     **/
     ...
     *
     * @return An array containing all classes reported as either predictions or labels.
     **/
     ...
     **/
    public void removeNull(String n) {
        nullLabels.remove(n);
    }
     ...
     * @return <code>true</code> iff <code>n</code> is one of the "null" labels.
     **/
    public boolean isNull(String n) {
        return nullLabels.contains(n);
    }


    /** Returns <code>true</code> iff there exist "null" labels. */
    public boolean hasNulls() {
        return nullLabels.size() > 0;
    }
     ...
     * @param h2 The second histogram, whose values will be added into the first's.
     **/
    protected void histogramAddAll(HashMap h1, HashMap h2) {
        for (Iterator I = h2.entrySet().iterator(); I.hasNext();) {
            Map.Entry e = (Map.Entry) I.next();
            histogramAdd(h1, (String) e.getKey(), ((Integer) e.getValue()).intValue());
        }
    }
     ...
     **/
    public int getLabeled(String l) {
        return histogramGet(goldHistogram, l);
    }
    ...
    /**
     * Returns the F<sub>beta</sub> score associated with the given label. F<sub>beta</sub> is
     * defined as: <blockquote> <i>F<sub>beta</sub> = (beta<sup>2</sup> + 1) * P * R</i> <i>/
     * (beta<sup>2</sup> * P + R)</i> </blockquote>
     *
    ...
    /**
     * Computes overall the overall statistics precision, recall, F<sub>1</sub>, and accuracy. Note
     * that these statistics are all equivalent unless "null" labels have been added.
     *
     * @return An array in which the first element represents overall precision, the second
     *         represents overall recall, then F1, and finally accuracy.
     **/
    public double[] getOverallStats() {
        return getOverallStats(1);
    }


    /**
     * Computes overall the overall statistics precision, recall, F<sub>beta</sub>, and accuracy.
     * Note that these statistics are all equivalent unless "null" labels have been added.
     *
     * @param b The value of beta.
     * @return An array in which the first element represents overall precision, the second
     *         represents overall recall, then F1, and finally accuracy.
    ...
    public double[] getOverallStats(double b) {
        ...
        if (hasNulls()) {
            ...
            result[2] = (b * b + 1) * result[0] * result[1] / (b * b * result[0] + result[1]);
        } else
    ...
    public void printPerformance(PrintStream out) {
        ...
        if (hasNulls())
            rowLabels[rows - 2] = "Overall";

        String[] columnLabels =
                new String[] {"Label", "Precision", "Recall", "F1", "LCount", "PCount"};
        ...
        for (int i = 0; i < allClasses.length; ++i) {
            ...
            table[i] = new Double[] {zero, zero, zero, new Double(labeled), new Double(predicted)};
        ...
        if (hasNulls()) {
            ...
            if (notNullPredicted > 0)
                table[rows - 2][0] = new Double(100 * notNullCorrect / (double) notNullPredicted);
            if (notNullLabeled > 0)
                table[rows - 2][1] = new Double(100 * notNullCorrect / (double) notNullLabeled);
            ...
            dashRows = new int[] {0, allClasses.length};

        double accuracy = totalPredicted == 0 ? 0 : 100 * totalCorrect / (double) totalPredicted;
        table[rows - 1] =
                new Double[] {new Double(accuracy), null, null, null, new Double(totalPredicted)};
