...
/**
 * This {@link ILPSolver} implements Egon Balas' zero-one ILP solving algorithm. It is a branch and
 * bound algorithm that can return the best solution found so far if stopped early. For more
 * information on the original algorithm, see <br>
 *
 * <blockquote> E. Balas. 1965. An Additive Algorithm for Solving Linear Programs with Zero-One
 * Variables. <i>Operations Research</i>, 13(4):517–546. </blockquote>
 *
 * @author Nick Rizzolo
 **/
public class BalasHook extends ZeroOneILPProblem implements ILPSolver {
    ...
    /**
     * Each element is <code>true</code> iff the corresponding inference variable's value in
     * {@link #solution} has been negated (which happens iff that variable initially had a negative
     * objective function coefficient).
     **/
    private boolean[] negated;

    /**
     * The current solution being evaluated in the intermediate stages of the algorithm.
     **/
    private int[] x;
    /**
     * The current values that, when added to the left hand sides of the corresponding constraints,
     * cause all constraints to be satisfied at equality during the intermediate stages of the
     * algorithm.
     **/
    private double[] slack;
    /**
     * A set of variables which must retain their current settings in <code>x</code> as the
     * algorithm continues processing.
     **/
     ...
     * this constructor is <code>true</code>.
     *
     ...
     * parameter to this constructor is <code>true</code>.
     *
     * @param f Whether or not to stop at the first feasible solution.
     * @param v Setting for the {@link #verbosity} level.
     **/
     ...
     * solve. The constraints in the problem are assumed to all be "less than or equal to"
     * constraints, and the actual (in)equality symbol is ignored during parsing.
     *
     * @param name The name of the file containing the textual representation of a 0-1 ILP problem.
     **/
    public BalasHook(String name) {
        this(name, ILPInference.VERBOSITY_NONE);
    }
    ...
    /**
     * Simply overrides {@link ZeroOneILPProblem#addConstraint(int[],double[],int,double)} so that
     * it calls {@link ZeroOneILPProblem#addConstraint(int[],double[],double)} thereby ignoring the
    ...
    /**
     * Given a potential solution, this method determines the values for the slack violates that
     * will satisfy our less-than constraints at equality.
     *
     ...
     *
     * @param z The value of the objective function with the current variable settings.
     * @return <code>true</code> iff a solution was found successfully.
     **/
    public boolean solve(final double z) {
        ...
        final double[] lhsNegative = constraintSatisfiability(violated, eligible, atEquality);
        if (lhsNegative == null)
            return false;
        ...
        for (boolean satisfiable = true; satisfiable;) {
            if (atEquality.size() > 0) {
                ...
                if (indexes == null)
                    indexes = sortVariablesByViolations(eligible);
                ...
                for (int i = 0; i < violatedSize && satisfiable; ++i) {
                    ...
                    for (int j = 0; j < newlyIneligible.size(); ++j) {
                        final int vIndex = Av.binarySearch(cIndex, newlyIneligible.get(j));
                        if (vIndex >= 0) {
                    ...
                    satisfiable = lhsNegative[i] - ZeroOneILPProblem.TOLERANCE <= slack[cIndex];
                    if (satisfiable
                            && Math.abs(slack[cIndex] - lhsNegative[i]) < ZeroOneILPProblem.TOLERANCE)
    ...
    /**
     * Determines which variables have a chance both to improve on the incumbunt solution and to
     * bring the current x closer to feasibility.
     *
     * @param z The value of the objective function with the current variable settings.
     * @param violated The set of violated constraints.
     * @return A vector of variables as described above.
     **/
    private IVector getEligibleVariables(final double z, final IVector violated) {
    ...
    /**
     * This method <i>attempts</i> to satisfy all specified constraints by turning on all eligible
     * variables that have a negative coefficient in any of them.
     *
     * <p>
     * If there are constraints satisfied at equality when their negative coefficient variables are
     * turned on, we know the only chance to satisfy them is to turn on all eligible variables with
     * negative coefficients in such constraints. This method does just that before making the
     * recursive call to {@link #solve(double)}.
     *
     * @param atEquality A vector of constraints which need to be satisfied.
     * @param z The value of the objective function with the current variable settings.
     * @param eligible A vector of variables which are eligible to be turned on.
     * @return <code>true</code> iff turning on all eligible variables as described above lead to a
     *         feasible solution.
     **/
    private boolean satisfyAll(final IVector atEquality, double z, final IVector eligible) {
    ...
    /**
     * For each violated constraint, this method determines whether it is individually satisfiable
     * given the eligible variables remaining. If all constraints are still satisfiable, the sums of
     * the negative coefficients on eligible variables for each constraint is returned. Otherwise,
     * <code>null</code> is returned.
     *
     * <p>
     * As a side effect, this method also determines which constraints can only be satisfied exactly
     * at equality and stores them in the given vector.
     *
     * @param violated The set of violated constraints.
     * @param eligible The set of variables still eligible to be turned on.
     * @param atEquality A vector into which is stored the set of constraints that can only be
     *        satisfied at equality.
     **/
    private double[] constraintSatisfiability(final IVector violated, final IVector eligible,
        ...
        for (int i = 0; i < violatedSize; ++i) {
            ...
            if (lhsNegative[i] - ZeroOneILPProblem.TOLERANCE > slack[index])
                return null;
            if (Math.abs(slack[index] - lhsNegative[i]) < ZeroOneILPProblem.TOLERANCE)
    ...
    /**
     * Computes a vector of indexes that, in effect, sorts the given variables according to how
     * violated the constraints would be if each were turned on independently. Ties are broken by
     * giving precedence to variables with smaller objective coefficients.
     *
     * @param eligible The variables to be sorted.
     * @return An array of <code>Integer</code> indexes pointing into the <code>eligible</code>
     *         vector.
     **/
    private int[] sortVariablesByViolations(final IVector eligible) {
        ...
        Sort.sort(indexes, new Sort.IntComparator() {
            public int compare(int i1, int i2) {
                if (Math.abs(violations[i1] - violations[i2]) < ZeroOneILPProblem.TOLERANCE) {
                ...
                if (violations[i1] < violations[i2])
                    return -1;
                return 1;
            }
        });
