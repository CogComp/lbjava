...
public abstract class BalasHook extends ZeroOneILPProblem implements ILPSolver {
    ...
    private double[] slack(int[] x) {
        ...
        for (int i = 0; i < Ac.size(); ++i) {
            ...
            if (Math.abs(rounded - result[i]) < TOLERANCE)
                result[i] = rounded;
        }
    ...
    private double[] constraintSatisfiability(final IVector violated, final IVector eligible,
        ...
        for (int i = 0; i < violatedSize; ++i) {
            ...
            if (lhsNegative[i] - TOLERANCE > slack[index])
                return null;
            if (Math.abs(slack[index] - lhsNegative[i]) < TOLERANCE)
    ...
    private int[] sortVariablesByViolations(final IVector eligible) {
        ...
        Sort.sort(indexes, new Sort.IntComparator() {
            public int compare(int i1, int i2) {
                if (Math.abs(violations[i1] - violations[i2]) < TOLERANCE) {
