...
public class FirstOrderDisjunction extends FirstOrderNAryConstraint {
    /**
     * If either of the arguments is itself a <code>FirstOrderDisjunction</code>, its contents are
     * flattened into this <code>FirstOrderDisjunction</code>.
     *
     * @param c1 One constraint to disjunct.
     * @param c2 Another constraint to disjunct.
     **/
    public FirstOrderDisjunction(FirstOrderConstraint c1, FirstOrderConstraint c2) {
        add(c1);
        add(c2);
    }
    ...
    public void add(FirstOrderConstraint c) {
        if (c instanceof FirstOrderDisjunction) {
            ...
            while (I.hasNext())
                add((FirstOrderConstraint) I.next());
        } else
            children.add(c);
    }
    ...
    public boolean evaluate() {
        for (Iterator I = children.iterator(); I.hasNext();)
            if (((FirstOrderConstraint) I.next()).evaluate())
                return true;
        return false;
    }
    ...
    public PropositionalConstraint propositionalize() {
        ...
        if (c.length == 1)
            return c[0].propositionalize();

        PropositionalDisjunction result =
                new PropositionalDisjunction(c[0].propositionalize(), c[1].propositionalize());
    ...
    /**
     * The hash code of a <code>FirstOrderDisjunction</code> is the sum of the hash codes of its
     * children.
     *
    ...
    /**
     * Two <code>FirstOrderDisjunction</code>s are equivalent when they are topologically
    ...
    public boolean equals(Object o) {
        if (!(o instanceof FirstOrderDisjunction))
            return false;
