 ...
 * distinguished only by what it takes as input. This class does not represent a syntax that appears
 * in the source - it is constructed only during semantic analysis.
 *
 * @author Nick Rizzolo
 **/
public class ClassifierType extends Type {
    ...
    public ClassifierType(Type i, ClassifierReturnType o, boolean l) {
        ...
        try {
            myClass = Class.forName("edu.illinois.cs.cogcomp.lbjava.classify.Classifier");
        } catch (Exception e) {
            System.err
                    .println("Class 'edu.illinois.cs.cogcomp.lbjava.classify.Classifier' not found.  "
                            + "Aborting.");
            System.exit(1);
        }
    }
    ...
    /**
     * Two <code>ClassifierType</code>s are equivalent when their input types match.
     *
    ...
    public boolean equals(Object o) {
        return o instanceof ClassifierType && input.equals(((ClassifierType) o).input);
    }
     ...
     **/
    public Object clone() {
        return new ClassifierType((Type) input.clone(), (ClassifierReturnType) output.clone(),
                learner);
    }
