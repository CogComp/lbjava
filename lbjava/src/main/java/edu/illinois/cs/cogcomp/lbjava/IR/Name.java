...
public class Name extends Expression {
    /**
     * (&not;&oslash;) These strings appeared with dots between them to form the name in the source.
     **/
    public String[] name;
    /**
    ...
    /**
     * Returns a new <code>Name</code> object that is the same as this <code>Name</code> object,
    ...
    public HashSet getVariableTypes() {
        ...
        if (name.length == 1)
            ...
            type = symbolTable.get(name[0]);

        if (type != null
                && (type instanceof ArrayType || type instanceof ReferenceType || type instanceof PrimitiveType))
    ...
    public boolean containsQuantifiedVariable() {
        ...
        if (types.size() == 0)
            return false;
        return ((Argument) types.iterator().next()).getType().quantifierArgumentType;
    }
    ...
    public boolean equals(Object o) {
        if (!(o instanceof Name))
            return false;
        Name n = (Name) o;
        if (n.name.length != name.length)
            return false;
        for (int i = 0; i < name.length; ++i)
            if (!name[i].equals(n.name[i]))
                return false;
        return true;
    }
     ...
     **/
    public Object clone() {
        return new Name(name, line, byteOffset);
    }
