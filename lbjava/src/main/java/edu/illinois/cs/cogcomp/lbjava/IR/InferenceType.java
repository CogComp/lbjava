 ...
 * from which the head can be found. This class does not represent a syntax that appears in the
 * source - it is constructed only during semantic analysis.
 *
 * @author Nick Rizzolo
 **/
public class InferenceType extends Type {
    ...
    public InferenceType(Type h, Type[] f) {
        ...
        try {
            myClass = Class.forName("edu.illinois.cs.cogcomp.lbjava.infer.Inference");
        } catch (Exception e) {
            System.err
                    .println("Class 'edu.illinois.cs.cogcomp.lbjava.infer.Inference' not found.  "
                            + "Aborting.");
            System.exit(1);
        }
    }
     ...
     *
     * @param h The head object's type.
     * @param f The array of head finder arguments from the inference declaration.
     **/
     ...
     * @return The type at location <i>i</i> in the <code>headFinderTypes</code> array member
     *         variable, or <code>null</code> if <i>i</i> is outside the bounds of that array.
     **/
    public Type getFinderType(int i) {
        if (i < 0 || i >= headFinderTypes.length)
            return null;
    ...
    /**
     * Two <code>InferenceType</code>s are equivalent when their head types match.
     *
     * @param o The object whose equality with this object needs to be tested.
     * @return <code>true</code> if the two objects are equal, and <code>false</code> otherwise.
     **/
    public boolean equals(Object o) {
        return o instanceof InferenceType && headType.equals(((InferenceType) o).headType);
    }
     ...
     **/
    public Object clone() {
        return new InferenceType((Type) headType.clone(), (Type[]) headFinderTypes.clone());
    }
