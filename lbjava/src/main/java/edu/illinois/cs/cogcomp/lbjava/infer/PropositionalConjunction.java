...
public class PropositionalConjunction extends PropositionalNAryConstraint {
    ...
    /**
     * If either of the arguments is itself a <code>PropositionalConjunction</code>, its contents
     * are flattened into this <code>PropositionalConjunction</code>.
     *
     * @param c1 One constraint to disjunct.
     * @param c2 Another constraint to disjunct.
     **/
    public PropositionalConjunction(PropositionalConstraint c1, PropositionalConstraint c2) {
        add(c1);
        add(c2);
    }
    ...
    /**
     * Same as <code>simplify()</code>, except this method gives the caller the ability to
     * optionally leave double implications that are immediate children of this conjunction in tact.
     *
     * @param d <code>true</code> iff double implications that are immediate children of this
     *        conjunction are to be left in tact.
     * @return A logically simplified version of this constraint.
     **/
    public PropositionalConstraint simplify(boolean d) {
        ...
        if (d) {
            for (Iterator I = children.iterator(); I.hasNext();) {
                ...
                if (c instanceof PropositionalDoubleImplication) {
                    PropositionalDoubleImplication di = (PropositionalDoubleImplication) c;
                    di.left = di.left.simplify();
                    di.right = di.right.simplify();

                    if (di.left.equals(di.right))
                        c = PropositionalConstant.True;
                    else if (di.left.equals(PropositionalConstant.False))
                        c = di.right.negate().simplify();
                    else if (di.left.equals(PropositionalConstant.True))
                        c = di.right;
                    else if (di.right.equals(PropositionalConstant.False))
                        c = di.left.negate().simplify();
                    else if (di.right.equals(PropositionalConstant.True))
                        c = di.left;
                    /*
                     * else if (di.right instanceof PropositionalNegation &&
                     * di.left.equals(di.right.getChildren()[0]) || di.left instanceof
                     * PropositionalNegation && di.right.equals(di.left.getChildren()[0])) c =
         ...
         * Integer(i)); if (terms[j].moreGeneralThan(terms[i])) toRemove.add(new Integer(j)); }
         * 
    ...
    public PropositionalConstraint DNF() {
        ...
        while (PropositionalDisjunction.increment(children, indexes)) {
            PropositionalConstraint combination =
                    new PropositionalConjunction(children[0][indexes[0]], children[1][indexes[1]]);
            for (i = 2; i < children.length; ++i)
                combination = new PropositionalConjunction(combination, children[i][indexes[i]]);
    ...
    public PropositionalConstraint factor() {
        ...
        while (best != null) {
            ...
            for (int i = 0; i < children.length - 1
                    && children[i] instanceof PropositionalDisjunction; ++i)
            ...
            if (best != null) {
                ...
                if (children[bestOther] instanceof PropositionalDisjunction) {
                    PropositionalConstraint disjunct1 =
                            ((PropositionalDisjunction) children[bestDisjunction]).subtract(best);
                    PropositionalConstraint disjunct2 =
                            ((PropositionalDisjunction) children[bestOther]).subtract(best);

                    toAdd =
                            new PropositionalDisjunction(toAdd, new PropositionalConjunction(
                                    disjunct1, disjunct2)).simplify();
                }
     ...
     * conjunctions; the intersection of a conjunction and some other constraint <i>c</i> is
     * <i>c</i> if <i>c</i> is contained in the conjunction and the empty set otherwise.
     *
    ...
    public PropositionalConstraint[] intersect(PropositionalConstraint c) {
        ...
        for (Iterator I = children.iterator(); I.hasNext();) {
            Object next = I.next();
            if (conjunction.children.contains(next))
                result.add(next);
        }
     ...
     *
     * @return A simplified constraint representing the distribution of the given disjunction over
     *         this conjunction.
     **/
    ...
    /**
     * The hash code of a <code>PropositionalConjunction</code> is the sum of the hash codes of its
     * children plus one.
     *
    ...
    /**
     * Two <code>PropositionalConjunction</code>s are equivalent when they are topologically
    ...
    public boolean equals(Object o) {
        if (!(o instanceof PropositionalConjunction))
            return false;
